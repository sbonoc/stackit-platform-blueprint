...
class DocsGenerationTests(unittest.TestCase):
    ...
    def test_docs_build_and_smoke(self) -> None:
        ...
        self.assertIn("`quality-sdd-check`", core_targets_content)
        self.assertIn("`quality-sdd-check-all`", core_targets_content)
        self.assertIn("`spec-impact`", core_targets_content)
        self.assertIn("`spec-evidence-manifest`", core_targets_content)
        self.assertIn("`spec-context-pack`", core_targets_content)
        self.assertIn("`spec-pr-context`", core_targets_content)
        self.assertIn("`quality-hardening-review`", core_targets_content)
        self.assertIn("`infra-port-forward-start`", core_targets_content)
        self.assertIn("`infra-port-forward-stop`", core_targets_content)
        self.assertIn("`infra-port-forward-cleanup`", core_targets_content)
        self.assertIn("`quality-docs-check-changed`", core_targets_content)
