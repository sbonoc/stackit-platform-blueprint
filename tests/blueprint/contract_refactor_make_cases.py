...
class MakeRefactorCases(RefactorContractBase):
    ...
    def test_makefile_template_supports_conditional_optional_targets(self) -> None:
        ...
        self.assertIn("{{PHONY_MANAGED_CACHE}}", makefile_template)
        ...
        self.assertIn("{{INFRA_ENV_GUARDED_MANAGED_CACHE}}", makefile_template)
        ...
        self.assertIn("{{TARGETS_MANAGED_CACHE}}", makefile_template)
        ...
        self.assertIn('"INFRA_ENV_GUARDED_MANAGED_CACHE=$phony_managed_cache" \\', makefile_renderer)
