...
class ScriptsRefactorCases(RefactorContractBase):
    ...
    def test_infra_audit_version_checks_local_helm_chart_pin_resolution(self) -> None:
        ...
        self.assertIn("run_cmd_with_retry_backoff", tooling)
        self.assertIn("HELM_REPO_UPDATE_RETRY_MAX_ATTEMPTS", tooling)
        ...
        self.assertIn("run_kubectl_with_active_access delete -f", tooling)
        self.assertIn('run_helm_with_active_access upgrade --install', tooling)
        self.assertIn('run_helm_with_active_access template', tooling)
        self.assertIn('run_helm_with_active_access uninstall', tooling)

    def test_port_forward_library_exposes_generic_lifecycle_primitives(self) -> None:
        port_forward = _read("scripts/lib/infra/port_forward.sh")
        self.assertIn("start_port_forward()", port_forward)
        self.assertIn("wait_for_local_port()", port_forward)
        self.assertIn("stop_port_forward()", port_forward)
        self.assertIn("cleanup_port_forwards()", port_forward)
        self.assertIn("PORT_FORWARD_WAIT_TIMEOUT_SECONDS", port_forward)
        self.assertIn("PORT_FORWARD_FORCE_KILL", port_forward)
        self.assertIn("port_forward_registry_prune_stale_entries()", port_forward)
        self.assertIn("port_forward_registry_file()", port_forward)
        self.assertIn("port_forward_registry_ensure()", port_forward)
    ...
    def test_blueprint_bootstrap_seeds_templates_from_single_list(self) -> None:
        ...
        self.assertIn('"docs/blueprint/architecture/decisions/README.md"', bootstrap)
        self.assertIn('"docs/blueprint/architecture/north_star.md"', bootstrap)
        self.assertIn('"docs/blueprint/architecture/tech_stack.md"', bootstrap)
        self.assertIn('"docs/blueprint/governance/spec_driven_development.md"', bootstrap)
    ...
    def test_quality_hooks_split_fast_and_strict_gates(self) -> None:
        ...
        self.assertIn("quality-docs-check-changed", hooks_fast)
        ...
        self.assertIn("quality-sdd-check-all", hooks_fast)
    ...
    def test_apps_bootstrap_keeps_only_canonical_app_dirs(self) -> None:
        ...
        self.assertIn("--app-descriptor-path", apps_bootstrap)
        self.assertIn("--component-image", apps_bootstrap)
        self.assertIn("--component-image-env-var", apps_bootstrap)
        self.assertIn('"backend-api=APP_RUNTIME_BACKEND_IMAGE"', apps_bootstrap)
        self.assertIn('"touchpoints-web=APP_RUNTIME_TOUCHPOINTS_IMAGE"', apps_bootstrap)
