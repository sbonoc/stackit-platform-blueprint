...
- [ ] proposal(epic-343-pre-promotion-evidence-gate): bundle of three instrumentation tasks + one Backstage 1-week spike + one spec-as-KO-vs-separate-KO 1-week spike that MUST run BEFORE Epic #343 is promoted from Draft to active backlog, gating the reframing intake referenced under `### P1 — Next Up`. Three instrumentation tasks (cheap, run continuously once shipped, cost ≈ a few hours of engineering each): (1) **re-litigation proxy** — bigram-overlap script (algorithm from `ADR-issue-364` § 4.2) measuring how much each new SDD spec overlaps with prior merged specs/ADRs; backfill against last 6 months of merged specs; promotion criterion ≥ 30% of new specs have ≥ 30% overlap; (2) **cross-reference distance** — for each merged PR, count directory traversals needed via repo grep to reach related decisions; promotion criterion average > 4 AND anecdotal navigation friction reported; (3) **"didn't know that existed" survey** — step03 sign-off adds one yes/no question; promotion criterion YES rate > 30% over 4 weeks. Two spikes: (4) **Backstage 1-week spike** — self-host Backstage on STACKIT SKE, feed SDD artifacts + C7 events + service catalog, evaluate parity; promotion criterion ≥ 70% feature parity achievable in ≤ 2 weeks of integration; (5) **spec-as-KO vs separate-KO 1-week spike** — implement both approaches against 5 real merged specs, compare query patterns + freshness behavior + schema friction + "no second HITL gate" property; promotion criterion the bifurcated direction (spec-as-KO for human-curated sources, separate KO for telemetry) is empirically cleaner. If ≥ 2 of the 3 instrumentation signals + both spikes pass criteria, Epic #343 is promoted and the v2 reframing intake is filed. Otherwise the Brain stays Draft and is revisited only when conditions change. Full design rationale + adversarial revisions that motivated this gate: `docs/blueprint/proposals/2026-06-22-epic-343-knowledge-fabric-reframing.md` § Addendum.
      rationale: the v1 proposal architecturally over-commits on Knowledge Compiler / Contract C9 / LLM-Wiki / federation / MCP-singular without evidence that the underlying problem ("memory and reasoning bottlenecks for the factory") is real at N=1, without verifying that vendor alternatives (Backstage, Apache AGE, pgvector, LlamaIndex, LangFuse — all CNCF-mature open-source, sovereignty-respecting) reduce custom-build scope substantially, and without prototyping the SDD-spec-as-Knowledge-Object pattern that would eliminate transformation drift on human-curated sources. The evidence gate runs these three checks BEFORE we invest 6–12 months in the wrong architecture. Sized as parked `on-scope: quality` because the instrumentation tasks ship independently of any Epic #343 commitment and the spike outcomes are reversible (no production code lands until v2 intake authoring).
