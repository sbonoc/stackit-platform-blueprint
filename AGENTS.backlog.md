...
- [x] (done 2026-06-22) **POST-MERGE one-shot (Factory — Epic #332, surfaces at PR #372 merge)** execute #361 parent-merge operator runbook — file 4 child issues (`#361.1` + `#361.2` + `#361.4` + `#361.5`) via `bash specs/2026-06-18-issue-361-orchestrator-service/file_children.sh`; append `#361.3` deferred-filing triggers via `bash specs/2026-06-18-issue-361-orchestrator-service/add_deferred_triggers.sh`; manually add the 5-checkbox `## Integration Acceptance Criteria` section to the `#361` issue body (per T-003 + FR-013 + Contract C4). Executed against `main` HEAD `c8c6f1a9` (PR #372 merge commit); children filed as #373 (#361.1, agent-ready) / #374 (#361.2) / #375 (#361.4) / #376 (#361.5); #361.3 deferred-filing entries injected under `### after: issue-335` + `### after: issue-336` via commit `54a39bb9`; Integration AC section appended to #361 body with 5 checkboxes AC-005..AC-009 and child-issue mapping.
...
- [ ] **FUTURE (Factory — Epic #343 reframing, blocked_by: Epic #332 100% stable + Epic #343 promotion + evidence-gate spike outcomes per `proposal(epic-343-pre-promotion-evidence-gate)` below)** Epic #343 reframing — evolve Epic #343 from "graph + vector knowledge layer" into a knowledge fabric. **Canonical strategic-direction doc (repo-tracked, status `strawman-for-evaluation` NOT a ratified decision):** `docs/blueprint/proposals/2026-06-22-epic-343-knowledge-fabric-reframing.md` — the doc has TWO sections: a v1 body (initial strawman from the user-supplied brief) AND a v1-addendum (same-day adversarial revisions that demote 6 load-bearing v1 decisions to hypotheses and introduce a bifurcated architecture). **Read the addendum FIRST** before drafting the intake — it SUPERSEDES the v1 body for any future intake author. v2 direction at a high level: (a) SDD specs ARE the Knowledge Objects (no separate envelope for human-curated sources); (b) bifurcated Compiler (tiny SDD Indexer + LlamaIndex-based Telemetry Compiler); (c) Backstage on STACKIT SKE for service catalog + human-facing surface; (d) Apache AGE + pgvector on STACKIT-managed Postgres; (e) Access Layer is multi-protocol (HTTP/GraphQL + agent tool-use), NOT MCP-singular; (f) LLM-Wiki + federation deferred to Phase 5+. v1 vocabulary (singular Knowledge Compiler + Contract C9 as one contract + MCP-singular + federation-Phase-1 + LLM-Wiki in MVP) is RETRACTED per the addendum. Do NOT pick up until all three pre-conditions met. Source brief: user-supplied "Enterprise Knowledge Fabric Amendment Brief" (2026-06-22), retained externally by the user and cited in the proposal doc's § Provenance. — https://github.com/sbonoc/stackit-platform-blueprint/issues/343
...
- [ ] proposal(epic-343-pre-promotion-evidence-gate): bundle that gates Epic #343 promotion from Draft to active backlog, composed of (a) signal output from issue **#378** (factory-observability instrumentation — three signals shipping independently under Epic #332, NOT a Brain prerequisite — re-litigation proxy + cross-reference distance + "didn't know that existed" survey) and (b) two 1-week spikes owned by THIS gate (Backstage on STACKIT SKE + spec-as-KO-vs-separate-KO prototype). Promotion criteria: re-litigation proxy ≥ 30% of new specs have ≥ 30% overlap with prior decisions; cross-reference distance average > 4 directories AND anecdotal navigation friction reported; "didn't know that existed" YES rate > 30% over 4 weeks; Backstage spike achieves ≥ 70% feature parity in ≤ 2 weeks of integration; spec-as-KO spike confirms bifurcated direction (spec-as-KO for human-curated, separate KO for telemetry) is empirically cleaner including the cross-source query friction measurement. **Date-bearing decision tracker: issue #379** (`decision(epic-343-evidence-gate): 2026-09-22 calendar-checkpoint decision — promote Epic #343 or explicit hold`) — that issue is the load-bearing mechanism for the calendar checkpoint; this backlog entry is INFORMATIONAL for the architectural reasoning + criteria. Without issue #379, the 2026-09-22 date here would be unenforceable because AGENTS.backlog.md trigger vocabulary (`after: <slug>`, `on-scope: <tag>`, `triage: next-session` per `docs/blueprint/governance/sdd_execution_guide.md`) has no date-based trigger and this entry would surface only when a `quality`-scoped intake happens to land — which is NOT guaranteed near 2026-09-22 (per Codex P2 review on PR #377 round 3). Full design rationale + adversarial revisions that motivated this gate: `docs/blueprint/proposals/2026-06-22-epic-343-knowledge-fabric-reframing.md` § Addendum.
      rationale: the v1 proposal architecturally over-commits on Knowledge Compiler / Contract C9 / LLM-Wiki / federation / MCP-singular without evidence that the underlying problem ("memory and reasoning bottlenecks for the factory") is real at N=1, without verifying that vendor alternatives (Backstage, Apache AGE, pgvector, LlamaIndex, LangFuse — all CNCF-mature open-source, sovereignty-respecting) reduce custom-build scope substantially, and without prototyping the SDD-spec-as-Knowledge-Object pattern that would eliminate transformation drift on human-curated sources. The evidence gate runs these checks BEFORE we invest 6–12 months in the wrong architecture. **Decoupling note (2026-06-22):** the three instrumentation tasks were extracted into standalone issue #378 under Epic #332 because they produce factory-observability signal useful regardless of whether Epic #343 ever ships — answering "are we drifting into duplicated specs?", "is repo navigation friction increasing?", and "how often are humans surprised by existing decisions?" independently. This gate CONSUMES #378's signal output rather than owning the work; if #378 ships and gives clean signal, this gate runs its two spikes (Backstage + spec-as-KO) and decides on 2026-09-22 via issue #379.
- [ ] proposal(quality-commit-autoclose-scan): commit-message auto-close-keyword scan against open parent/epic issues — PR #372 merge inadvertently auto-closed parent #361 on 2026-06-22T10:28:34Z because commit `22264f51`'s body contained the substring `auto-closed #361` (the word `auto-closed` ends with the `closed` token followed by ` #361`, which GitHub's auto-close parser matches as a closing keyword via the standard `closed #N` regex). The existing AC-013 regex in `tests/blueprint/test_issue_361_file_children_script.py` correctly detects this pattern when applied (verified by `uv run python3 -c "import re; print(re.compile(r'\\b(close[ds]?|fix(?:e[ds])?|resolve[ds]?):?\\s+#361\\b', re.IGNORECASE).search('auto-closed #361'))"` returning a Match object) — but it only guards GENERATED content (children body templates, deferred-trigger backlog rationale). It does NOT scan commit messages. Fix: add a `quality-pr-body-and-commit-autoclose-scan` CI check that, given a target PR + a configurable list of must-not-auto-close issue numbers (sourced from PR body markers like `Tracks #N` OR a `.github/no-auto-close-issues.yml` config), runs the AC-013 regex against (a) PR title + body, (b) every commit message subject + body on the PR branch, and FAILS the build if any unintended match is found. The check should support a per-PR override comment (`#allow-auto-close: #N1,#N2`) for legitimate cases like child PRs that DO close their own issue. Authored against the AGENTS.md SDD lifecycle as a quality-hardening change; consumer repos inherit the check via the existing quality-* hook surface.
      rationale: the immediate operator action (reopen #361) takes seconds, future parent coordination PRs are rare (1 in the last 6 months), and the check requires the `must-not-auto-close` allowlist mechanism to be designed carefully to avoid breaking normal child PRs — but the same trap WILL recur on any future #333/#343 promotion intake that decomposes into children unless this scan moves into CI. Sized as parked `on-scope: quality` so it surfaces the next time quality-hook scope is extended.
...
- [ ] (parked) proposal(issue-361-orchestrator-service): file #361.3 — RabbitMQ subscriber + OpenHands API client + work loop (trigger: after: issue-336)
      trigger: after: issue-336
      rationale: #361.3 is the `architectural-layer: infrastructure — external-runtime clients` boundary of the #361 5-child decomposition (canonical vocabulary per ADR-issue-337-light-decomposition-policy § Allowed boundary types). Filing deferred per Q-1 (Option B) until BOTH #335 AND #336 reach spec-complete (this entry surfaces when #336 resolves; the operator MUST also verify #335 is spec-complete before filing #361.3, since the child's OpenHands API client depends on #335's Agent Server API surface being concrete). If #336 has resolved but #335 has not, defer filing until the sibling entry on `### after: issue-335` also surfaces — do NOT file #361.3 against incomplete contracts (Codex P2 review of PR #372). Parent spec: specs/2026-06-18-issue-361-orchestrator-service/. When filing, the #361.3 body MUST cite parent spec FR-016 — the PR scope MUST include `git rm` of `specs/2026-06-18-issue-361-orchestrator-service/file_children.sh` and `specs/2026-06-18-issue-361-orchestrator-service/add_deferred_triggers.sh` (verified by AC-012 / T-211 to be authored in #361.3). Per parent spec FR-017, the #361.3 PR body MUST cite parent #361 as `Tracks #361` (informational); GitHub auto-close keywords (`Closes` / `Fixes` / `Resolves`) MUST NOT target #361. Parent close is a deliberate human action after the Contract C4 Integration AC checkboxes are ticked.
...
- [ ] (parked) proposal(issue-347-human-sdd-c7-symmetry): widen `scripts/lib/sdd/c7_emit.py` to derive `outcome_details.touches_services[]` for `local-cli` emissions from the work-item `spec.md` Implementation Stack Profile + the `## Contract Changes` section at emission time (mirroring the orchestrator's derivation path). Currently the helper builds only the minimum C7 envelope plus caller-supplied extension fields; `design-contracts.md` § C7 (amended 2026-06-19 per PR #372 strategic-alignment audit) standardized `touches_services` as required-on-every-event for the orchestrator but softened the `local-cli` requirement on PR #372 7th-review per Codex P2-1 because the helper was not updated in lock-step (mandating it would have rendered every `local-cli` C7 event contract-violating until the helper is widened). Fix: extract a shared `derive_touches_services(spec_path: Path) -> list[str]` function callable from both the orchestrator and the local-cli helper (the orchestrator already needs this logic for its own `#361.2` extension-field population); wire it into `EmitC7EventUseCase.build()` so the helper auto-populates the field without the caller needing to pass it; update `design-contracts.md` § C7 to promote the `local-cli` requirement to required-on-every-event once the helper ships; extend `tests/sdd/test_c7_emit*.py` to assert auto-derivation against a fixture spec + that emission never fails when the spec is missing optional sections.
      trigger: after: issue-347
      rationale: same parking pattern as the six-input `event_id` widening above — `#347` is the canonical owner of the `local-cli` helper, and the contract change MUST land atomically with the helper update to avoid dead-on-arrival C7 events. The orchestrator-side derivation logic is owned by `#361.2` (extension-field population); the shared function could live in either ticket's scope, but parking the request under `#347` keeps the helper-side scope cohesive. Cross-reference `#361.2` when the work is picked up.
...
- [ ] (parked) proposal(epic-343-central-brain): #343-perspective review of the C7 emission shape before Epic #343 leaves Draft — verify the C7 minimum fields + extension fields that landed on PR #372 strategic audit 2026-06-19 (`token_usage[expert_slug].routing_key` sub-key + `outcome_details.touches_services[]` extension field) satisfy the Brain's graph-traversal needs (Journey A spec→PR recall; Journey C SRE incident→PR correlation by service touch). Also verify the `expert_slug_blueprint` / `expert_slug_extension` two-sub-enum shape (F-12 amendment 2026-06-19 per PR #372 5th-review) is correctly modeled in the Brain's graph schema. NOTE: the originally-proposed `local-cli` six-input `event_id` form + `decision_author` / `branch_sha` extension fields were REVERTED on PR #372 5th-review pass per Codex P2-1 (helper-side implementation was missing; would have rendered every prior `local-cli` C7 event contract-violating); both are parked separately under `### after: issue-347` and will rejoin the Brain-review scope only after `#347` lands the helper-side change. Confirm or amend the shape BEFORE Epic #343 promotes from Draft to active backlog so the ingestion-pipeline schema doesn't need to do gymnastics to consume historical events.
...
- [ ] (parked) proposal(epic-343-central-brain): legacy flat-`expert_slug` C7 payload normalization at Brain ingest. Historical local-cli C7 events emitted from human SDD work BEFORE the 2026-06-19 F-12 two-sub-enum amendment carry a flat `expert_slug` field in `outcome_details.expert_verdicts[]` (visible in `artifacts/c7/*.jsonl` — confirmed by `grep -h '"expert_slug"' artifacts/c7/*.jsonl` returning real production payloads). Post-amendment producers MUST emit the new two-sub-enum form (EXACTLY ONE OF `expert_slug_blueprint` OR `expert_slug_extension`) and the per-invocation `ExpertVerdict` (ADR-364 § 6) + compact `ExpertVerdictSummary` (ADR-364 § 9) + per-skill `expert_verdicts[]` schemas in all 7 panel-related SKILL.md files reject the flat form via `additionalProperties: false`. The Brain ingestion pipeline MUST therefore normalize the legacy shape (`expert_slug: <slug>` → `expert_slug_blueprint: <slug>` when the value matches the sealed 8-item baseline enum; otherwise treat as unknown legacy slug and log for human triage) BEFORE feeding events to the per-row schema validator. Fix: in the Brain ingest pipeline's pre-validation normalization step (Journey A intake), apply the rewrite rule above for any event whose `outcome_details.expert_verdicts[]` rows contain a flat `expert_slug` key; emit a normalization-applied audit log entry per rewritten event so the Brain's evolution timeline can attribute pre/post-amendment data correctly. Originally written as a per-row schema "MUST tolerate" clause across 7 SKILL.md files + ADR-364 § 6/§ 9 + design-contracts § C7 on PR #372 4th–8th review passes; that clause was structurally impossible (`additionalProperties: false` rejects unknown fields before any tolerance rule can run) so it was relocated to this Brain-ingest-layer entry on PR #372 11th-review per Codex P2-2.
      trigger: after: epic-343-promote
      rationale: separation of concerns — the per-row schema validates LIVE per-invocation expert outputs (no live producer emits the legacy form post-amendment) while the Brain ingest pipeline is the canonical legacy-payload normalization layer. Without this entry, when the Brain pipeline is authored, it will silently drop or reject historical events instead of normalizing them, losing the per-expert provenance the Brain needs for ADR-recall (Journey A) and cross-team awareness queries. Pre-amendment payload examples are committed and discoverable at `artifacts/c7/2026-06-02-issue-360-factory-personas-skills.jsonl`, `artifacts/c7/2026-06-02-issue-364-factory-expert-persona-panel.jsonl`, `artifacts/c7/2026-06-17-issue-368-factory-cost-telemetry-routing-fixture.jsonl`, and `artifacts/c7/2026-06-18-issue-361-orchestrator-service.jsonl` (this PR's own emissions before the amendment landed mid-PR).
...
- [ ] (parked) proposal(issue-361-orchestrator-service): file #361.3 — RabbitMQ subscriber + OpenHands API client + work loop (trigger: after: issue-335)
      ...
      rationale: #361.3 is the `architectural-layer: infrastructure — external-runtime clients` boundary of the #361 5-child decomposition (canonical vocabulary per ADR-issue-337-light-decomposition-policy § Allowed boundary types). Filing deferred per Q-1 (Option B) until BOTH #335 AND #336 reach spec-complete (this entry surfaces when #335 resolves; the operator MUST also verify #336 is spec-complete before filing #361.3, since the child's RabbitMQ subscriber depends on #336's trigger queue topology being concrete). If #335 has resolved but #336 has not, defer filing until the sibling entry on `### after: issue-336` also surfaces — do NOT file #361.3 against incomplete contracts (Codex P2 review of PR #372). Parent spec: specs/2026-06-18-issue-361-orchestrator-service/. When filing, the #361.3 body MUST cite parent spec FR-016 — the PR scope MUST include `git rm` of `specs/2026-06-18-issue-361-orchestrator-service/file_children.sh` and `specs/2026-06-18-issue-361-orchestrator-service/add_deferred_triggers.sh` (verified by AC-012 / T-211 to be authored in #361.3). Per parent spec FR-017, the #361.3 PR body MUST cite parent #361 as `Tracks #361` (informational); GitHub auto-close keywords (`Closes` / `Fixes` / `Resolves`) MUST NOT target #361. Parent close is a deliberate human action after the Contract C4 Integration AC checkboxes are ticked.
