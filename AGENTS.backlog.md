...
- [ ] P1 (Consumer upgrade flow): Issues #180 + #181 — `upgrade_shell_behavioral_check`: false positives on case-label `|` alternation and array literal bare-words (#180, P1) and `_EXCLUDED_TOKENS` incomplete — blueprint runtime functions and common OS tools flagged as unresolved (#181, P2); bundle into one work item as they affect the same component. Gate is unreliable until both are resolved. **In progress**: `specs/2026-04-24-issue-179-180-181-185-186-187-upgrade-correctness/`
- [ ] P1 (Consumer upgrade flow): Issue #179 — `upgrade_reconcile_report`: `conflicts_unresolved` bucket incorrectly includes files that have already been resolved; consumers receive a wrong conflict count and may act on stale data. **In progress**: `specs/2026-04-24-issue-179-180-181-185-186-187-upgrade-correctness/`
- [ ] P1 (Consumer upgrade flow): Issue #185 — upgrade planner silently skips new blueprint files not enumerated in `required_files` or `blueprint_managed_roots`; uncovered files produce no warning and the validate gate does not enforce coverage. **In progress**: `specs/2026-04-24-issue-179-180-181-185-186-187-upgrade-correctness/`
- [ ] P1 (Consumer upgrade flow): Issue #186 — `upgrade_fresh_env_gate`: gate passes on exit code only; file-state divergence check between clean worktree and working tree is not implemented, so the gate can report PASS while producing different output files. **In progress**: `specs/2026-04-24-issue-179-180-181-185-186-187-upgrade-correctness/`
- [ ] P2 (Consumer upgrade flow): Issue #187 — `render_ci_workflow`: generated `ci.yml` omits `permissions:` block; GITHUB_TOKEN inherits implicit write access on orgs with non-restrictive defaults. **In progress**: `specs/2026-04-24-issue-179-180-181-185-186-187-upgrade-correctness/`
