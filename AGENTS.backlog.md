...
- [ ] P1 (Runtime auth correctness): Issue #234 — `parse_literal_pairs()` splits `RUNTIME_CREDENTIALS_SOURCE_SECRET_LITERALS` on commas, silently truncating values containing commas (data URIs, base64 payloads); ESO source secret never created, all ExternalSecrets `NotReady`. Fix: newline-only `while IFS= read -r pair` loop; comma-separated input rejected with `log_warn`. Breaking change (Option B). **In progress**: `specs/2026-04-28-issue-234-literal-pairs-newline-format/`, PR #235.
- [ ] P1 (Make target ergonomics): Issue #241 (bug) — GNU Make override warnings when consumer re-defines blueprint-generated targets; expose `?=` override-point variables (`SPEC_SCAFFOLD_DEFAULT_TRACK`, `BLUEPRINT_UPLIFT_STATUS_SCRIPT`) in `blueprint.generated.mk` so consumers customise target behaviour without full target re-definition. Individual PR. on-scope: blueprint **In progress**: `specs/2026-04-30-issue-241-make-override-warnings/`, PR #242 — SPEC_READY, plan refined.
- [ ] P1 (Accessibility compliance): Issues #238 + #239 + #240 — three-slice combined PR: (1) #238: embed WCAG 2.1 AA into SDD spec lifecycle — `NFR-A11Y-001` in `spec.md`, `T-Axx` task block in `tasks.md`, hardening checklist section, WCAG SC column in `traceability.md`, `quality-spec-pr-ready` UI-layer validation; (2) #239: `touchpoints-test-a11y` + `apps-a11y-smoke` make targets, `axe_page_scan.mjs` Playwright runner, shared `axe_preset.ts` enforcing WCAG 2.1 AA ruleset and `attachTo: document.body`; (3) #240: `docs/platform/accessibility/acr.md` consumer-seeded scaffold, `quality-a11y-acr-check` staleness gate, `consumer_fitness_status` integration, PR Packager ACR checklist for UI-layer specs. on-scope: quality, skills
