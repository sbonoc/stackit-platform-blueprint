...
- [ ] P1 (Accessibility compliance): Issues #238 + #239 + #240 — three-slice combined PR: (1) #238: embed WCAG 2.1 AA into SDD spec lifecycle — `NFR-A11Y-001` in `spec.md`, `T-Axx` task block in `tasks.md`, hardening checklist section, WCAG SC column in `traceability.md`, `quality-spec-pr-ready` UI-layer validation; (2) #239: `touchpoints-test-a11y` + `apps-a11y-smoke` make targets, `axe_page_scan.mjs` Playwright runner, shared `axe_preset.ts` enforcing WCAG 2.1 AA ruleset and `attachTo: document.body`; (3) #240: `docs/platform/accessibility/acr.md` consumer-seeded scaffold, `quality-a11y-acr-check` staleness gate, `consumer_fitness_status` integration, PR Packager ACR checklist for UI-layer specs. on-scope: quality, skills **In progress**: `specs/2026-04-30-issue-238-239-240-a11y-compliance/`, PR #243 (Draft). SPEC_READY=false pending Q-1 (layer conditionality) and Q-2 (ACR integration path).
