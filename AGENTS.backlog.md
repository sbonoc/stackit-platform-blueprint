...
- [ ] (parked) proposal(issue-383-384-385-386-366-395-v1122-bugfixes): consumer migration guide for removed required vars — make blueprint-upgrade-consumer enhancement to detect now-optional vars in blueprint/repo.init.env and emit a migration warning
      trigger: after: #167
...
- [ ] (parked) proposal(issue-383-384-385-386-366-395-v1122-bugfixes): pnpm lockfile regeneration — docs/pnpm-lock.yaml was generated by pnpm@10 (lockfileVersion 9.0); regeneration under pnpm@11 produces lockfileVersion 10
      trigger: on-scope: docs
      rationale: --ignore-scripts is a safe working fix; surfaces when any developer bumps pnpm or changes docs dependencies
...
- [x] (done) proposal(quality-commit-autoclose-scan): commit-message auto-close-keyword scan against open parent/epic issues — shipped as issue #380, merged PR #381. `quality-pr-commit-autoclose-check` pre-push hook scans PR title/body + branch commit messages for GitHub auto-close keywords targeting `Tracks #N` protected issues; supports `#allow-auto-close: #N` override; fallback to `.github/no-auto-close-issues.yml`; inherited by consumer repos via bootstrap-template sync.
- [ ] (parked) proposal(quality-commit-autoclose-scan): multi-repo consumer PR CI enforcement (CI gate, not just pre-push) — pre-push is the primary safety layer for this cut; consumer repo workflow changes require separate intake
      trigger: on-scope: quality
      rationale: parked at PR #381 step07 publish triage; pre-push hook ships the safety layer; CI gate requires per-consumer workflow changes
- [x] (rejected) proposal(quality-commit-autoclose-scan): inline per-line override token — rejected at PR #381 closure. Per-PR override covers all known real cases; inline token adds regex + parsing complexity with no current use case.
- [x] (rejected) proposal(quality-commit-autoclose-scan): stale issue-number detection — rejected at PR #381 closure. Adds a second GitHub API call violating NFR-PERF-001 (≤ 1 API call per invocation); no proportional safety gain.
