...
1. Initialize repository identity:
   ...
   - When `BLUEPRINT_UPGRADE_ALLOW_DELETE=true` is set, the apply phase evaluates a three-layer prune guard stack before deleting any file absent in the blueprint source: (1) **Contract ownership** — files classified as `consumer_seeded`, `source_only`, or `init_managed` are never pruned; (2) **Consumer workload guard** — non-kustomization YAML files directly under `infra/gitops/platform/base/apps/` are never pruned (O(1) fast path); (3) **Kustomization-ref guard** — any file whose basename appears in the `resources:` or `patches:` list of a `kustomization.yaml` co-located in the same directory is classified `consumer-kustomization-ref` and skipped, protecting consumer-renamed manifests in overlay trees such as `infra/gitops/platform/environments/local/`; on a malformed `kustomization.yaml` the guard defaults to unprotected and emits a `warning:` to stderr. The apply artifact `summary` includes `consumer_kustomization_ref_count` for operational visibility.
