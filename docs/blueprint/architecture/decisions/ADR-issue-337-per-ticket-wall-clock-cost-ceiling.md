...
- **Check-order on every persona-invocation trigger.** #336 MUST evaluate FR-006 rerun-cap exhaustion BEFORE evaluating the FR-007 per-ticket ceiling, so quality-signal escalation (`factory-escalated`) fires before budget-signal pause (`factory-paused-ceiling`) when both conditions are simultaneously true on the same invocation. Rationale: the rerun cap is a quality-signal whose remediation (re-scope) is the precondition for the budget signal being interpretable; if both labels are applied to the same ticket in the opposite order, an operator who clears `factory-paused-ceiling` first re-triggers a doomed rerun that immediately re-hits the cap.
- **FR-001 lightweight-tier step03 ↔ FR-006 rerun-cap interaction.** Per FR-006's per-step rerun cap definition, `blueprint-sdd-step03-spec-complete` (routed to the lightweight tier per [`ADR-issue-337-llm-model-router-policy.md`](ADR-issue-337-llm-model-router-policy.md)) carries a step03 rerun cap identical to every other step; step03 failures contribute to the per-ticket ceiling on the same accumulator as every other step. The lightweight-tier routing does NOT create a cheaper-step exemption from either ceiling.
