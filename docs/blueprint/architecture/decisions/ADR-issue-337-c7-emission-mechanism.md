...
**Amended-by:** [`ADR-issue-364-expert-persona-model.md`](ADR-issue-364-expert-persona-model.md) — adds the non-required extension field `outcome.details.expert_verdicts[]` to C7 events. The eleven required fields, the three-emitter rule (`orchestrator` | `webhook-handler` | `local-cli`), and the `event_id` derivation are preserved verbatim — the amendment is purely additive (subscribers that omit the field MUST be tolerated). Per ADR-issue-364 § 2, the `persona` field on orchestrator-emitted events now carries the *draft-producing skill basename* (e.g., `blueprint-sdd-step01-intake`) rather than a stage-persona basename; per-expert attribution is carried on `outcome.details.expert_verdicts[]`. The field name `persona` is not renamed (FR-017(b) seal preserved); see `design-contracts.md` § C7 for the updated description.

## Context

[`design-contracts.md`](../../autonomous-factory/design-contracts.md) § Contract C7 pins **what** must be emitted (the eleven-field lifecycle event schema), **where** (a durable, replayable bus — STACKIT Managed RabbitMQ stream queues per FR-013 / Q-5), and **when** (every persona phase transition during autonomous execution). It deliberately leaves **how** open, on the premise that the implementation mechanism is a Phase 1 detail.
...
| C7 ingestion audit invariant detects, for a paired `phase: implement` and `phase: agent-pr-review` event on the same `ticket_id`, that the `model` field on the `agent-pr-review` event equals the `model` field on the `implement` event | `outcome: rejected`, `rejection_reason: rotation-violation` | FR-008 |

**Webhook handler event_id discriminator.** Every C7 event emitted by the webhook handler MUST populate the non-required extension field `webhook_event_key` (permitted by C7's `additionalProperties: true`) and include it in the `event_id` hash per the five-input variant declared in `design-contracts.md` § Contract C7. For rows in the table above whose triggering condition is an inbound GitHub webhook payload, `webhook_event_key` MUST be the `X-GitHub-Delivery` UUID — GitHub preserves this UUID across redeliveries, so subscriber-side dedupe still absorbs webhook-handler retries while distinct user actions produce distinct `event_id` values. For rows whose triggering condition is an internal audit invariant with no inbound GitHub webhook (the `rotation-violation` row), `webhook_event_key` MUST be `sha256(rejection_reason|outcome|trigger_timestamp_rfc3339)` from the handler-internal trigger context. The discriminator prevents collisions when the webhook handler legitimately emits multiple distinct events on the same `(ticket_id, phase, rerun_round)` tuple (e.g., successive `integration-criteria-bot-tick-blocked` rejections from repeated bot-tick attempts on the same phase; a `rerun-cap-exceeded` rejection followed by a separately-triggered `rotation-violation` rejection on the same phase).

**Webhook handler persona/model sentinel values.** All rows in the table above are emitted by deterministic non-LLM code — there is no persona invocation and no LiteLLM model resolution. The eleven-field C7 minimum schema requires `persona` and `model` as non-null strings, so webhook-handler emissions MUST populate them with **sealed sentinel values**: `persona: webhook-handler` (the deterministic webhook handler emitter is itself the actor of record) and `model: n/a` (no LLM produced the event). The field types remain `string` and the required-set remains the eleven-field minimum — the sentinel rule preserves the sealed schema without introducing nullable types or emitter-conditional required-set variants. For the `rotation-violation` row, the violating model identifier (the model whose appearance on both paired events caused the equality) MAY additionally be carried as the non-required extension field `violating_model` (permitted by `additionalProperties: true`) for audit traceability; this is NOT part of the eleven-field minimum and subscribers MUST NOT depend on it for the FR-008 audit predicate (which fires from the paired orchestrator events).
...
- Meta-ADR: [`ADR-issue-337-factory-phase-0-foundations.md`](ADR-issue-337-factory-phase-0-foundations.md)
- Design contracts: `docs/blueprint/autonomous-factory/design-contracts.md` § Contract C7 (eleven-field lifecycle event schema, emission-transport rule, emission-mechanism rule), § Contract C8 (consumer-shipped surface)
