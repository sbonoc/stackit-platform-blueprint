...
**Amended-by:** [`ADR-issue-364-expert-persona-model.md`](ADR-issue-364-expert-persona-model.md) — the FR-008 reviewer-heterogeneity audit invariant continues to pair the `phase: implement` C7 event with the `phase: agent-pr-review` event on the same `ticket_id`, but the predicate is **re-expressed at the model-family layer** against the per-panel routing-key set (the C7 extension field `outcome_details.routing_keys`), not against the agent-pr-review event's aggregate `model` field. Under the expert-panel model the AI PR reviewer is an 8-expert panel dispatched by `blueprint-sdd-step08-agent-pr-review` (Contract C3 row 8, structured-disagreement convergence); the agent-pr-review event's aggregate `model` field carries only the lead voice's routing key (ADR-issue-364 § 4.4 / § 4.5), so the pre-amendment string-equality predicate ("implement.model differs from agent-pr-review.model") is unsatisfiable under the panel model and is superseded. The amended two-leg predicate is: (a) `model_family(implement.model)` is NOT the sole family represented in the agent-pr-review event's `outcome_details.routing_keys` set, AND (b) the routing-key set carries ≥ 2 distinct families (panel-disjointness). Both legs operate at the canonical-family layer via `model_family(s)` normalization (defined in `../../autonomous-factory/design-contracts.md` § C7 extension-field vocabulary) so the audit is robust against LiteLLM routing-prefix vs minimum-schema `model` aliasing (e.g., `claude-sonnet-4-6` and `anthropic/claude-sonnet-4-6` both normalize to `sonnet`). The Decision / Pairings / Enforcement sections below remain authoritative for the family-rotation intent (Opus ↔ Sonnet at the family layer is preserved); the routing-key-set / family-normalization mechanics are how that intent is now expressed against the panel event. Per-expert capacity sizing and routing-key shape land in ticket #335.
