...
- #339 Contract C6 `### Blueprint instance` (Q-2) and C7 `### Blueprint instance` (Q-3) are resolved to concrete values in the same PR cycle; C7 `### Open Decisions` (durable-bus pick) is resolved; C8 enumerates the ten ADRs as `stable` consumer-shipped surface so every consumer-instance factory inherits the same architectural baseline.
