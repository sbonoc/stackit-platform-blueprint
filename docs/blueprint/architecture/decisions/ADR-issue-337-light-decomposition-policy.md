...
Decomposition along any other axis is FORBIDDEN. Mixed-axis decompositions (e.g., "two children by bounded context and three by layer") are FORBIDDEN by `blueprint-ticket-decompose-light` invocations — the single axis is what makes the children's relationship to the parent inspectable.

**Multi-axis exception for manually-authored parent coordination specs (amended 2026-06-19 per PR #372 strategic audit).** When a parent ticket's scope legitimately spans two distinct boundary axes AND the decomposition is being authored MANUALLY in a parent coordination spec (i.e., NOT through `blueprint-ticket-decompose-light` runtime invocation), the human spec author MAY decompose along two axes provided that: (a) every child cites EXACTLY ONE canonical boundary type (no child carries a hybrid label), (b) the parent spec's FR explicitly enumerates each axis used + the rationale for needing more than one, (c) the parent spec's `## Notes for Child Intake` includes a clear statement that the multi-axis decomposition was authored manually and does NOT establish a precedent for the skill (the skill remains single-axis-only). This exception exists because `blueprint-ticket-decompose-light` is itself authored by the orchestrator-decomposition work (Epic #332 Phase 1) — the skill cannot exist until the meta-decomposition that produces it merges, and that meta-decomposition is exactly the class of work that legitimately spans runtime architecture AND governance/expert-panel scope. Worked example: PR #372 (parent of `#361`) decomposes into 4 children under `architectural-layer` (runtime) + 1 child under `bounded-context: expert-panel` (governance/persona contract — the persona file IS its own bounded context distinct from the orchestrator runtime per ADR-issue-364 three-layer model). The skill's `large-decomposable` runtime classification MUST continue to fall back to escalate-or-single-axis behaviour; the multi-axis exception is human-authored only.
...
- Phase 1 ticket #336 enforces the parent-stays-open rule (parent issue MUST NOT be auto-closed by child PR merges) and the bot-tick-block on `## Integration Acceptance Criteria`. GitHub does not expose a cancellable pre-close hook on the linked-PR auto-close behavior — `closes #N` / `fixes #N` / `resolves #N` references in a merged PR body atomically close the referenced issue on default-branch merge with no interception point — so the parent-stays-open enforcement MUST be realized as a **two-control composition** of GitHub-native primitives:
  1. **Primary control (preventive — required status check on child PRs).** #336 MUST register a required status check (e.g., `factory/parent-stays-open`) on every child PR whose body references a parent issue carrying a `## Integration Acceptance Criteria` section (parent detection via #339 Contract C2's child-spec convention). The check parses the child PR body on `pull_request.opened`, `pull_request.edited`, and `pull_request.synchronize` events; if any GitHub-recognized closing keyword (`close|closes|closed|fix|fixes|fixed|resolve|resolves|resolved` per [GitHub's linked-PR docs](https://docs.github.com/en/issues/tracking-your-work-with-issues/using-issues/linking-a-pull-request-to-an-issue#linking-a-pull-request-to-an-issue-using-a-keyword)) precedes a `#N` reference to a parent issue with an unticked IAC checkbox, the check MUST fail with a message instructing the author to rewrite the reference as `Refs #N` (or any non-closing-keyword form). Because branch protection blocks merge while the required check is failing, no auto-close trigger is ever armed when the child merges.
  2. **Compensating control (corrective — issues.closed reopen + C7 rejection).** As defense-in-depth for parents that get closed by any path the primary control does not cover (manual close by an admin, GraphQL `closeIssue` mutation, a closing keyword merged before the check ran, a forced merge bypassing branch protection), #336 MUST listen for `issues.closed` webhook events on every parent issue carrying a `## Integration Acceptance Criteria` section. On every such event with one or more unticked IAC checkboxes at the moment of close, #336 MUST immediately reopen the parent via the GitHub API, post a PR-style comment naming each unticked checkbox, and emit a C7 event with `outcome: rejected` and `rejection_reason: parent-closed-before-integration-criteria-ticked` as a non-required extension field permitted by C7's `additionalProperties: true`.

  Children themselves close normally on their own PR merge; only the parent's auto-close path is blocked (primary control) or compensated (corrective control).
...
- C7 lifecycle event stream (#339 Contract C7) carries `boundary_type` and `boundary_value` as **non-required extension fields** (permitted by C7's `additionalProperties: true`) on decomposition events; per-axis distribution is auditable from the event stream. These fields are NOT part of the eleven-field minimum schema — #333 implementers MUST populate them on every decomposition event the orchestrator emits, and subscribers MUST treat their presence as guaranteed only on `phase: plan-slicer` events from decomposition runs (not as a schema-wide invariant).
