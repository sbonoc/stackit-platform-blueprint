...
**`escalate` routing rule.** `escalate`-classified tickets MUST NOT receive factory execution. The factory bot MUST apply the `factory-escalate-human` label and post a PR/issue comment naming which threshold dimension triggered escalation. No `agent-ready` label may be applied to an `escalate`-classified ticket; if applied, the trigger MUST be no-op'd and a C7 lifecycle event emitted with `phase: intake` and `outcome: human-handoff` (the valid C7 enum values for an at-intake escalation). The specific blocking reason (e.g., `escalation_reason: escalate-class-blocked`) MAY be carried in an additional non-required field per the C7 schema's `additionalProperties: true` allowance — additional fields are permitted; the eleven required minimum fields MUST NOT be renamed or have their types changed.
...
- C7 lifecycle event stream (#339 Contract C7) carries triage events with `outcome: success` when the classification function runs to completion (the schema-valid outcome of a triage emission); the triage class itself is carried as a `triage_class` extension field with one of `{small, medium, large-decomposable, escalate}` (permitted by C7's `additionalProperties: true`). The dashboard derived from FR-012/Q-4 (`stackit-managed-grafana`) reports class-distribution-over-time as a calibration signal by grouping on the `triage_class` extension field.
