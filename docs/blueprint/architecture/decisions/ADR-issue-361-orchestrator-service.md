...
|---|---|---|---|---|
5 children decompose along two canonical axes per the ADR-issue-337-light-decomposition-policy "Multi-axis exception for manually-authored parent coordination specs" (amended 2026-06-19 per PR #372 strategic-alignment audit): 4 under `architectural-layer` (runtime), 1 under `bounded-context: expert-panel` (governance/persona contract). Every child carries EXACTLY ONE canonical boundary type.
