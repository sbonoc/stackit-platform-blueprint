...
- Open Decisions RESOLVED in PR #340 on 2026-05-28: Q-1 picked `stackit-factory-bot` (reserve+verify in #334); Q-2 picked four flat sign-off team slugs `@sbonoc/factory-{product,architecture,security,operations}` plus separate per-bounded-context teams (concrete provisioning + bounded-context list in #337); Q-3 picked STACKIT-managed Grafana via the existing observability module (concrete dashboard URLs in #337). Downstream tickets #334 and #337 retain non-closure conditions tied to the deferred concrete-value provisioning, gated under each section's `### Open Decisions` subsection. None of these defer to a consumer repo.
- Q-4 (LiteLLM access configuration field name and location in `contract.yaml`) RESOLVED in this PR: lives at `spec.factory_contract.litellm` under a new top-level `spec.factory_contract:` block, grouping every factory per-instance value (`litellm`, `bot_identity`, `codeowners`, `dashboard`, `version`) under one subtree so `/blueprint-consumer-upgrade` (per #342) walks a single namespace for drift detection.
