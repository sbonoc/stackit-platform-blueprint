...
- **Composition with FR-003 trigger authorization.** Solo-operator mode adjusts the SoD multi-author floor only; it does NOT relax the FR-003 trigger-authorization gate (`agent-ready` label may only be applied by a member of the repository-declared `agent_ready_allowlist_team` per [`ADR-issue-337-trigger-authorization-model.md`](ADR-issue-337-trigger-authorization-model.md)). Repos selecting `solo_operator: true` MUST therefore ensure their `agent_ready_allowlist_team` includes the solo operator, or the solo operator cannot apply `agent-ready` labels — the two controls compose AND-wise: a triggering action requires allowlist membership; a merge requires SoD satisfaction by the active path.
