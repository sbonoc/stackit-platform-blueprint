...
The panel-disjointness audit invariant (from amended `ADR-issue-337-reviewer-model-heterogeneity.md`) MUST be satisfied: within a single step's panel, the set of LiteLLM routing keys actually used MUST contain at least 2 distinct values for any panel of size ≥ 2 (i.e., the panel MUST NOT collapse to a single shared routing key across all experts) for the same `(ticket_id, phase, rerun_round)` tuple. Pairwise uniqueness across all experts is NOT required (and is infeasible for the all-8 panels at step01 and step08 when only three model tiers — Haiku, Sonnet, Opus — are routable); duplication within a panel is permitted as long as the panel as a whole carries ≥ 2 distinct keys. Step03's panel-of-1 is exempt (the invariant only binds panels of size ≥ 2). The Sonnet/Opus tier mix (4 Sonnet, 4 Opus baseline — with documentation-discipline overriding to Haiku at step01/step08) satisfies this automatically for the all-8 panels.
