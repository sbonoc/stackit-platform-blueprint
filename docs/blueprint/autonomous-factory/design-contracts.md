...
| `outcome_details.token_usage` | `map` of `object` keyed by `expert_slug` | `orchestrator` and `local-cli` for all panel-dispatched C7 events (steps with ≥ 2 experts) | Per-expert input/output token counts collected from the LiteLLM API `usage` response block for each dispatched expert. Shape per key: `{ "input_tokens": integer ≥ -1, "output_tokens": integer ≥ -1 }` — the value domain is `[-1, ∞)` where values ≥ 0 represent actual counts and -1 is the sentinel for unavailable usage data. When the LiteLLM API returns no `usage` block for a given expert, the orchestrator MUST record `{ "input_tokens": -1, "output_tokens": -1 }` for that expert so emission never fails on missing metadata. Subscribers MUST NOT reject events that carry sentinel values. The field is additive — pre-#368 subscribers MUST tolerate events that include it; post-#368 subscribers MUST NOT reject events that omit it. Standardized by ADR-issue-368. |
...
| `outcome_details.ticket_token_summary` | `object` | `orchestrator` and `local-cli` for `phase: agent-pr-review` (step08 close-out events only) | Per-ticket aggregate token roll-up computed at step08 emit time by reading all prior phase events for the same `ticket_id` from `artifacts/c7/<slug>.jsonl`. Required keys and computation rules: (1) `total_input_tokens` (integer ≥ 0) — sum of `outcome_details.token_usage[expert_slug].input_tokens` across all panel-dispatched phase events for the `ticket_id`; treat sentinel `-1` as `0` (conservative undercount). (2) `total_output_tokens` (integer ≥ 0) — sum of `outcome_details.token_usage[expert_slug].output_tokens` across all panel-dispatched phase events for the `ticket_id`; treat sentinel `-1` as `0`. (3) `total_expert_step_instantiations` (integer ≥ 0) — count of individual expert-slug entries present in `outcome_details.token_usage` maps across all panel-dispatched phase events for the `ticket_id` (i.e., the total number of expert invocations across all steps, including reruns). Emission MUST NOT fail when some per-phase token_usage entries carry sentinel values. The roll-up lives on the step08 event so per-ticket cost is queryable without joining across phase events (NFR-OBS-001). The field is additive — pre-#368 subscribers MUST tolerate events that include it; post-#368 subscribers MUST NOT reject events that omit it. Standardized by ADR-issue-368. |
...
**Expert-persona extensibility scope (F-12 clarification).** The `extensible` tier for persona rows above means **shadowing** of the 8 blueprint expert personas — replacing a blueprint expert's `PERSONA.md` with a consumer-authored version that preserves the same `<expert-slug>` directory name and satisfies all sections required by ADR-issue-364 § 3. It does NOT permit adding net-new expert slugs that are outside the 8-slug enum sealed in ADR-issue-364 § 6. The `expert_slug` enum on C7 `outcome_details.expert_verdicts[]` and on the `ExpertVerdict` / `ExpertVerdictSummary` schemas is sealed at the 8 blueprint slugs. A consumer that requires a domain-specific expert MUST either (a) shadow one of the 8 blueprint experts under the consumer namespace with a sector-specific worldview, or (b) file a blueprint amendment via the #339 sign-off cycle to add the slug to the sealed enum. Widening the enum unilaterally in a consumer instance MUST be rejected by the ingest pipeline's schema validation.
