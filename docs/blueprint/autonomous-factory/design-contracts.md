...
```json
{
  ...
  "properties": {
    "event_id":         { "type": "string",  "description": "Deterministic dedupe key with **emitter-conditional derivation**: `sha256(ticket_id|phase|rerun_round|emitter)` for `emitter: orchestrator`; `sha256(ticket_id|phase|rerun_round|emitter|webhook_event_key)` for `emitter: webhook-handler` (the five-input variant; see § Contract C7 emission idempotency for the `webhook_event_key` sourcing rule); `sha256(ticket_id|phase|rerun_round|emitter|decision_author|branch_sha)` for `emitter: local-cli` (the six-input variant amended 2026-06-19 per PR #372 strategic-alignment audit — without these two discriminators, two different human operators running the same SDD step on the same `ticket_id` from different branches would collide on the four-input form, and the Central Brain ingest pipeline (#343) would dedupe distinct decisions into one graph node, losing the per-decision provenance Brain Journey A requires for ADR-recall and cross-team awareness). Populated by the emitter (orchestrator #333 or webhook handler #336 or `local-cli` helper #347) before publish; identical retries MUST produce the identical `event_id` so subscribers can dedupe at-least-once delivery." },
...
| `outcome_details.expert_verdicts` | `array` of `object` | `orchestrator` and `local-cli` (when an expert panel was dispatched) | Per-expert **compact summary** array merged by the orchestrator from the panel invocations dispatched for the phase per the Contract C3 dispatch matrix. Each row is the `ExpertVerdictSummary` shape pinned in ADR-issue-364 § 9: required keys `expert_slug` (**enum — MUST be one of** `product-pragmatist` \| `boundary-hawk` \| `security-paranoid` \| `data-privacy` \| `test-quality-sceptic` \| `operability-sre` \| `documentation-discipline` \| `performance-cost-aware` — the enum is sealed at 8 slugs; values outside this set MUST be rejected by the ingest pipeline), `verdict` (enum `pass` \| `revise` \| `block`), and `findings_count` (integer ≥ 0). The full per-finding payload (`category`, `summary`, `evidence_ref`, `severity`) lives in workspace artifacts referenced by the event's sibling extension field `evidence_uri` (see row below) — it MUST NOT be inlined into the C7 event so dashboards and ingest pipelines can query counts without loading artifact payloads. Aggregation precedence is `block` > `revise` > `pass` per ADR-issue-364 § 6. The field is additive — subscribers that predate ADR-issue-364 MUST tolerate events that omit it; subscribers MUST NOT reject events that omit it. |
...
| `outcome_details.touches_services` | `array` of `string` | `orchestrator` and `local-cli` on every phase-boundary event | The set of service identifiers the work item touches, read from the work-item `spec.md` Implementation Stack Profile + the `## Contract Changes` section (API contract surfaces) at emission time. Service identifier format: free-form short slug owned by the consumer repo's service registry (e.g., `orders-api`, `users-svc`, `factory-orchestrator`); the orchestrator/local-cli MUST emit the empty array `[]` when the work item touches no service (e.g., docs-only or governance-only work items). This field enables the Central Brain (#343) Journey C SRE-incident-correlation query (`spec → ticket → PR → service → SLO → incident → owner_team`); without it the Brain cannot answer "what shipped in the last 24h touching `orders-api` or its dependencies" without joining git diff + Grafana label sets, which is expensive and lossy at ingest time. Required for the Brain to leave Draft per `epic #343`. The field is additive — pre-Brain subscribers MUST tolerate events that include it; the Brain ingestion pipeline MUST tolerate events that omit it (older events emitted before this amendment) and treat omission as "service set unknown." Standardized 2026-06-19 per PR #372 strategic-alignment audit; consumer repos may extend the per-instance service-identifier vocabulary in their `contract.yaml` per Contract C8 § FR-018 consumer-namespace pattern. |
| `outcome_details.decision_author` | `string` | `local-cli` only | GitHub login of the human operator who invoked the SDD step skill that emitted this event. Populated from the operator's `gh auth status` GitHub identity at emit time, OR from the env var `BLUEPRINT_DECISION_AUTHOR` if set (CI environments without a real GitHub login). MUST NOT be the factory bot login (Contract C5) — `local-cli` events represent human-assisted decisions, not factory-bot decisions; if the configured author equals the factory bot login, emission MUST fail with a clear error. Required for the `local-cli` six-input `event_id` derivation (see `event_id` description above) — without this discriminator, two different human operators running the same SDD step on the same `ticket_id` from different branches would collide on the four-input form and the Central Brain (#343) ingest pipeline would dedupe distinct decisions into one graph node. Standardized 2026-06-19 per PR #372 strategic-alignment audit. |
| `outcome_details.branch_sha` | `string` | `local-cli` only | Short git SHA (7 chars) of the local branch HEAD at emit time. Populated by the local-cli helper from `git rev-parse --short HEAD`. Required for the `local-cli` six-input `event_id` derivation alongside `decision_author` — together they discriminate distinct decisions on the same `(ticket_id, phase, rerun_round)` tuple authored on different branches or by different operators. Standardized 2026-06-19 per PR #372 strategic-alignment audit. |
...
| `rejection_reason` | `string` | `orchestrator` and `local-cli` (events with `outcome: rejected`) | Machine-readable cause of the rejection. Reserved value `missing-expert-verdict` is emitted when one or more dispatched expert verdicts were absent, malformed, or unparseable after retry per ADR-issue-364 § 6.1; the orchestrator pairs this value with a stub `outcome_details.expert_verdicts[]` row carrying `verdict: "block"` and `findings_count: 0`, with the synthetic finding `{category: panel-incomplete, severity: critical}` written to the workspace artifact at `evidence_uri`. Other reserved values may be added by future ADRs; consumers MUST tolerate unknown values. Absent on `outcome: success` events. |
...
**Expert-persona extensibility scope (F-12 clarification, amended 2026-06-19 per PR #372 strategic-alignment audit to introduce the extension-enum pattern).** The `extensible` tier for persona rows above means **shadowing** of the blueprint expert personas — replacing a blueprint expert's `PERSONA.md` with a consumer-authored version that preserves the same `<expert-slug>` directory name and satisfies all sections required by ADR-issue-364 § 3. The C7 `expert_slug` field on `outcome_details.expert_verdicts[]` and on the `ExpertVerdict` / `ExpertVerdictSummary` schemas is composed of two sub-enums following an additive extension pattern (amended to avoid the per-addition `#339` sign-off cost that compounds across Epic #332's remaining roadmap):
