...
1. Initialize repository identity:
   ...
   - The plan phase audits every file in the blueprint source tree against the contract (`required_files`, `init_managed`, `conditional_scaffold`, `feature_gated`, `blueprint_managed_roots`, `source_only`). Any uncovered file is emitted as a WARNING and counted in `upgrade_plan.json` under `uncovered_source_files_count`. The validate gate hard-fails (`make blueprint-upgrade-consumer-validate` exits non-zero) when `uncovered_source_files_count > 0`; this indicates a blueprint contract authoring error that must be resolved before any consumer apply can proceed. The `feature_gated` class covers paths that are conditionally present based on a feature flag (e.g., `apps/catalog`); declare them under `spec.repository.ownership_path_classes.feature_gated` in `blueprint/contract.yaml`. Files in the source tree matching any `source_artifact_prune_globs_on_init` pattern are counted as covered in the plan-phase audit and excluded from the uncovered WARNING list. `source_only` entries accept three forms: an exact relative file path, a directory prefix (trailing `/`), or a glob pattern containing `*`; `make infra-validate` evaluates glob and directory entries via `fnmatch` against the consumer file list rather than checking for file existence.
   - When `BLUEPRINT_UPGRADE_ALLOW_DELETE=true` is set, the apply phase evaluates a four-layer prune guard stack before deleting any file absent in the blueprint source: (1) **Contract ownership** — files classified as `consumer_seeded`, `source_only`, or `init_managed` are never pruned; (2) **App descriptor guard** — any path declared as a component manifest in `apps/descriptor.yaml` is classified `consumer-app-descriptor` and skipped (canonical app-ownership source going forward); (3) **Consumer workload guard** (deprecated bridge — removal trigger `after: consumer-app-descriptor-adoption`) — non-kustomization YAML files directly under `infra/gitops/platform/base/apps/` are never pruned (O(1) fast path); (4) **Kustomization-ref guard** — any file whose basename appears in the `resources:` or `patches:` list of a `kustomization.yaml` co-located in the same directory is classified `consumer-kustomization-ref` and skipped, protecting consumer-renamed manifests in overlay trees such as `infra/gitops/platform/environments/local/`; on a malformed `kustomization.yaml` the guard defaults to unprotected and emits a `warning:` to stderr. The apply artifact `summary` includes `consumer_app_descriptor_count` and `consumer_kustomization_ref_count` for operational visibility. When an existing consumer lacks `apps/descriptor.yaml`, the apply phase additionally writes `artifacts/blueprint/app_descriptor.suggested.yaml` (a starting-point descriptor derived from the apps `kustomization.yaml`) without modifying the consumer working tree; adoption is explicit.
   - After a successful (no-conflict) 3-way merge on a `.tf` file, the apply phase scans for duplicate top-level named block declarations (e.g. `variable "opensearch_enabled" {}`). Byte-identical duplicates are auto-removed and the result is recorded as `merged-deduped`; a `deduplication_log` array in the apply artifact lists each removed block with its type, label, and file path. Non-identical duplicates emit a conflict artifact (`result=conflict`) rather than writing syntactically invalid Terraform to disk. The apply artifact `summary` includes a `tf_dedup_count` counter.
   ...
   - The postcheck includes a **behavioral validation gate** that runs `bash -n` and a grep-based symbol-resolution check on every `result=merged` shell script produced by the apply phase. If a merged script has a syntax error or calls a function whose definition was silently dropped during merge, the postcheck fails and reports the findings in `artifacts/blueprint/upgrade_postcheck.json` under the `behavioral_check` key (fields: `status`, `files_checked`, `syntax_errors` `[{file, error}]`, `unresolved_symbols` `[{file, symbol, line}]`, `extra_excluded_count`). The symbol resolver traverses the **full transitive source chain** of each checked script using BFS with a cycle guard (frozenset of visited absolute paths), so functions defined in files sourced transitively at any depth are correctly resolved. The resolver excludes: case-label alternation tokens (`token|)` and `token | )`), bare-word elements inside `local`/`declare`/`readonly`/`typeset` array initializer blocks (`var=(`), and all standard shell builtins, common external commands, and blueprint bootstrap-chain runtime functions (including `tar`, `pnpm`, `uv`, `validate`, `blueprint_require_runtime_env`, `ensure_file_from_template`, and others). Consumer repos can extend the exclusion set without patching blueprint code by declaring additional token names in `blueprint/contract.yaml` under `spec.upgrade.behavioral_check.extra_excluded_tokens`; these are merged with the base set per invocation and the count is recorded in `extra_excluded_count`.
   ...
   - After apply, run `make blueprint-upgrade-fresh-env-gate` to verify CI equivalence. The gate creates a temporary git worktree from `HEAD`, seeds the upgrade artifact directory (`artifacts/blueprint/`) from the working tree into the worktree (since gitignored files are absent from a fresh checkout by design), then runs `make infra-validate` and `make blueprint-upgrade-consumer-postcheck` inside the clean worktree. A divergence report is written to `artifacts/blueprint/fresh_env_gate.json`. The gate compares SHA-256 checksums of all non-volatile files under `artifacts/blueprint/` between the clean worktree and the working tree; any file whose checksum differs appears in the `divergences` array as `{"path": "...", "worktree_checksum": "...", "working_tree_checksum": "..."}`. Volatile artifacts that embed per-run absolute repository paths (`upgrade_validate.json`, `required_files_status.json`, `upgrade_postcheck.json`, `upgrade_preflight.json`, `upgrade_reconcile_report.json`, `fresh_env_gate.json`) are excluded from the checksum comparison to prevent false-positive divergences. The gate exits non-zero when divergences are non-empty even if both make targets exit 0, meaning CI would produce different artifact content from the local upgrade run.
