...
**Expert-persona extensibility scope (F-12 clarification, amended 2026-06-19 per PR #372 strategic-alignment audit to introduce the extension-enum pattern).** The `extensible` tier for persona rows above means **shadowing** of the blueprint expert personas — replacing a blueprint expert's `PERSONA.md` with a consumer-authored version that preserves the same `<expert-slug>` directory name and satisfies all sections required by ADR-issue-364 § 3. The legacy single `expert_slug` field on `outcome_details.expert_verdicts[]` (and on the `ExpertVerdict` / `ExpertVerdictSummary` schemas) is REPLACED by EXACTLY ONE OF two distinct sub-enum fields per row, following an additive extension pattern (amended to avoid the per-addition `#339` sign-off cost that compounds across Epic #332's remaining roadmap). Pre-amendment events that carry a flat `expert_slug` field MUST be tolerated and treated identically to `expert_slug_blueprint` when the value matches the blueprint-baseline enum:
