...
| `outcome_details.ticket_token_summary` | `object` | `orchestrator` and `local-cli` for `phase: agent-pr-review` (step08 close-out events only) | Per-ticket aggregate token roll-up computed at step08 emit time by reading all prior phase events for the same `ticket_id` from `artifacts/c7/<slug>.jsonl`. Required keys: `total_input_tokens` (integer ≥ 0), `total_output_tokens` (integer ≥ 0), `total_expert_step_instantiations` (integer ≥ 0). Computation rule: sum `outcome_details.token_usage[expert_slug].input_tokens` across all panel-dispatched phase events for the `ticket_id`; treat sentinel value `-1` as `0` (conservative undercount — emission MUST NOT fail on missing metadata). The roll-up lives on the step08 event so per-ticket cost is queryable without joining across phase events (NFR-OBS-001). The field is additive — pre-#368 subscribers MUST tolerate events that include it; post-#368 subscribers MUST NOT reject events that omit it. Standardized by ADR-issue-368. |
