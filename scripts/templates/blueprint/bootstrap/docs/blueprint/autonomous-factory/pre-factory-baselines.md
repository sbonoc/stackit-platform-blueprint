...
The proxy (`fix(` title prefix) captures defect-fix PRs but does not differentiate "fix of a defect introduced by a prior PR in this window" from "fix of a pre-existing defect from before the window." Per FR-014's measurement scope, the comparable factory metric is the **AI-reviewer first-attempt rejection rate from C7 events** (per the instrumentation plan), which uses the C7 `outcome: rejected` enum value on events with `phase: agent-pr-review` (the sealed C7 outcome enum is `success | rejected | retried | human-handoff` per `design-contracts.md` § Contract C7); once live, this is directly comparable to a forward measurement of the same proxy in the same SDD-era workflow.
