...
resolve_optional_module_execution() {
  ...
  postgres:plan | postgres:apply)
    if is_stackit_profile; then
      ...
      optional_module_execution_set "fallback_runtime" "helm" "$(rendered_module_helm_values_file "postgres")"
    else
      log_fatal "unsupported BLUEPRINT_PROFILE=$BLUEPRINT_PROFILE"
    fi
    ;;
  ...
  object-storage:plan | object-storage:apply)
    if is_stackit_profile; then
      ...
      optional_module_execution_set "fallback_runtime" "helm" "$(rendered_module_helm_values_file "object-storage")"
    else
      log_fatal "unsupported BLUEPRINT_PROFILE=$BLUEPRINT_PROFILE"
    fi
    ;;
  ...
  secrets-manager:plan | secrets-manager:apply | secrets-manager:destroy)
    ...
    ;;
  managed-cache:plan | managed-cache:apply)
    if is_stackit_profile; then
      optional_module_execution_set "provider_backed" "foundation_contract" "$(stackit_terraform_layer_dir foundation)"
    elif is_local_profile; then
      optional_module_execution_set "fallback_runtime" "helm" "$(rendered_module_helm_values_file "managed-cache")"
    else
      log_fatal "unsupported BLUEPRINT_PROFILE=$BLUEPRINT_PROFILE"
    fi
    ;;
  ...
  kms:plan | kms:apply)
    if is_stackit_profile; then
      ...
      optional_module_execution_set "fallback_runtime" "helm" "$(rendered_module_helm_values_file "kms")"
    else
      log_fatal "unsupported BLUEPRINT_PROFILE=$BLUEPRINT_PROFILE"
    fi
    ;;
