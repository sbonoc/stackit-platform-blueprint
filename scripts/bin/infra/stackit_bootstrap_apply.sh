...
start_script_metric_trap "infra_stackit_bootstrap_apply"

usage() {
  cat <<'USAGE'
Usage: stackit_bootstrap_apply.sh

Runs terraform apply for the STACKIT bootstrap layer.
USAGE
}
...
run_terraform_action_with_backend apply "$bootstrap_dir" "$backend_file" "$var_file" "${tf_var_args[@]}"

state_file="$(
  write_state_file "stackit_bootstrap_apply" \
...
log_info "stackit bootstrap apply state written to $state_file"
