...
- App Dockerfile scaffold and runtime deployment fix (Issues #111, #112):
  ...
  - `infra/gitops/platform/base/apps/backend-api-deployment.yaml` updated: image changed from `python:3.13.9` to `ghcr.io/example-org/platform-blueprint-backend:0.1.0`; hardcoded `command:` override removed (CMD is now defined in the Dockerfile). `touchpoints-web-deployment.yaml` updated: image changed from `nginx:1.29.2` to `ghcr.io/example-org/platform-blueprint-touchpoints:0.1.0`. Both bootstrap template copies synced.
