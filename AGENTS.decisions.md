...
- Epic #343 (Central Brain) reframing direction recorded 2026-06-22 as a `strawman-for-evaluation` proposal (NOT a ratified architecture decision):
  - Canonical strategic-direction doc: `docs/blueprint/proposals/2026-06-22-epic-343-knowledge-fabric-reframing.md` — the v1 body captures the initial strawman from a user-supplied "Enterprise Knowledge Fabric Amendment Brief"; the v1-addendum at the bottom carries the same-day adversarial revisions that demote six v1 load-bearing decisions to hypotheses and introduce a bifurcated architecture (SDD specs ARE Knowledge Objects, no separate envelope for human-curated sources). Read the addendum FIRST when picking up the reframing work.
  - Phase 0 evidence gate composed of two strands: (A) factory-observability signals owned by issue #378 under Epic #332 (re-litigation proxy + cross-reference distance + "didn't know that existed" survey — ship and emit independently of any Brain commitment), (B) two 1-week spikes owned by the gate itself (Backstage on STACKIT SKE + spec-as-KO-vs-separate-KO prototype). Date-bearing decision tracker: issue #379 (`decision(epic-343-evidence-gate): 2026-09-22 calendar-checkpoint decision`) is the load-bearing mechanism — `AGENTS.backlog.md` trigger vocabulary has no date-based trigger, so the backlog entry is informational and the GitHub issue carries the date. Calendar checkpoint **2026-09-22**; if criteria not met, EXPLICITLY hold and revisit at 2026-12-22 or sooner via a successor tracker issue.
  - Six v1 → v2 revisions ratified in the addendum: (R1) bifurcate Compiler into SDD Indexer + Telemetry Compiler; (R2) split Contract C9 into tiny C2 front-matter extension + new C9 for telemetry only; (R3) SDD specs ARE Knowledge Objects (no separate envelope for human-curated sources); (R4) demote LLM-Wiki to Phase 5+; (R5) demote federation to Phase 5+ (single-tenant MVP); (R6) replace MCP-singular with multi-protocol Access Layer.
  - Sovereignty-respecting open-source target stack: Backstage (CNCF) + Apache AGE + pgvector on STACKIT-managed Postgres + LlamaIndex + LangFuse.
  - Parent backlog entry: `AGENTS.backlog.md` § P1 — Next Up → `FUTURE (Factory — Epic #343 reframing, blocked_by: Epic #332 100% stable + Epic #343 promotion + evidence-gate spike outcomes ...)`. Gate entry: `AGENTS.backlog.md` § on-scope: quality → `proposal(epic-343-pre-promotion-evidence-gate)`.
  - This entry is governance-of-record per `AGENTS.md` line 112 ("Update `AGENTS.decisions.md` when scope/contracts/priorities change") in response to Codex P2 PR #377 review noting the strategic redirection was discoverable only via the proposal doc + backlog entry, not the decision log itself.
