...
| 2b | Conflict triage: emit `artifacts/blueprint/upgrade_triage.json` with `recommended_action` per conflict. When conflicts are present, run `make blueprint-upgrade-consumer-resolve` as a **post-pipeline step** (not automated by the pipeline) to auto-apply `take_source`/`take_target` rows and print the residual table of `human_required` rows |
