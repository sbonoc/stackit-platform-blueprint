...
| NFR-SEC-001 | SDD-C-009, SDD-C-013 | | `language: system` + `make` entry in hook | `.pre-commit-config.yaml`, `scripts/templates/blueprint/bootstrap/.pre-commit-config.yaml` | Hook stanza review | ADR §Non-Functional Architecture Notes | No remote exec; no credential exposure |
