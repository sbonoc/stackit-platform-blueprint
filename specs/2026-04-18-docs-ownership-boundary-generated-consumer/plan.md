# Implementation Plan

## Implementation Start Gate
- Implementation tasks MUST remain unchecked until `SPEC_READY=true`.
- If required inputs are missing, add `BLOCKED_MISSING_INPUTS` in `spec.md` and keep the gate closed.

## Constitution Gates (Pre-Implementation)
- Simplicity gate:
  - Keep initial implementation scope minimal and explicit.
  - Avoid speculative future-proof abstractions.
- Anti-abstraction gate:
  - Prefer direct framework primitives over wrapper layers unless justified.
  - Keep model representations singular unless boundary separation is required.
- Integration-first testing gate:
  - Define contract and boundary tests before implementation details.
  - Ensure realistic environment coverage for integration points.
- Positive-path filter/transform test gate:
  - For any filter or payload-transform logic, at least one unit test MUST assert that a matching fixture value returns a record.
  - Positive-path assertions MUST verify relevant output fields remain intact after filtering/transform.
  - Empty-result-only assertions MUST NOT satisfy this gate.
- Finding-to-test translation gate:
  - Any reproducible pre-PR finding from smoke/`curl`/deterministic manual checks MUST be translated into a failing automated test first.
  - The implementation fix MUST turn that test green in the same work item.
  - If no deterministic automation path exists, publish artifacts MUST record the exception rationale, owner, and follow-up trigger.

## Delivery Slices
1. Slice 1: implement repo-mode-aware platform seed sync and generated-consumer template-orphan cleanup in `sync_platform_seed_docs.py`.
2. Slice 2: implement repo-mode-aware summary sync/check behavior in `sync_runtime_identity_contract_summary.py` and `sync_module_contract_summaries.py`, then add regression tests.

## Change Strategy
- Migration/rollout sequence:
  - add repo-mode and required-seed-file allowlist resolution in platform seed sync script
  - add generated-consumer orphan cleanup path (sync + check modes)
  - add generated-consumer template-skip logic to runtime identity + module summary sync scripts
  - add regression tests for generated-consumer behavior and keep template-source checks intact
  - update SDD/ADR/governance artifacts
- Backward compatibility policy:
  - keep existing make targets and docs sync entrypoints unchanged
  - keep template-source synchronization semantics strict and deterministic
- Rollback plan:
  - revert docs sync script changes and associated tests
  - rerun `make infra-validate` and `make quality-hooks-fast`

## Validation Strategy (Shift-Left)
- Unit checks:
  - `python3 -m unittest tests.blueprint.test_quality_contracts`
  - `python3 -m unittest tests.docs.test_docs_generation`
  - `python3 -m unittest tests.docs.test_orchestrate_sync`
- Contract checks:
  - `make infra-validate`
- Integration checks: not required for script/docs-only scope.
- E2E checks: not required for script/docs-only scope.

## App Onboarding Contract (Normative)
- Required minimum make targets:
  - `apps-bootstrap`
  - `apps-smoke`
  - `backend-test-unit`
  - `backend-test-integration`
  - `backend-test-contracts`
  - `backend-test-e2e`
  - `touchpoints-test-unit`
  - `touchpoints-test-integration`
  - `touchpoints-test-contracts`
  - `touchpoints-test-e2e`
  - `test-unit-all`
  - `test-integration-all`
  - `test-contracts-all`
  - `test-e2e-all-local`
  - `infra-port-forward-start`
  - `infra-port-forward-stop`
  - `infra-port-forward-cleanup`
- App onboarding impact: no-impact | impacted (select one)
- App onboarding impact: no-impact
- Notes: no app onboarding contract surface changed.

## Documentation Plan (Document Phase)
- Blueprint docs updates: add ADR + update work-item artifacts; update governance/backlog/decision notes for docs ownership boundary execution.
- Consumer docs updates: none (ownership mechanics only).
- Mermaid diagrams updated: ADR scope/context diagrams.
- Docs validation commands:
  - `make docs-build`
  - `make docs-smoke`

## Publish Preparation
- PR context file:
  - `pr_context.md`
- Hardening review file:
  - `hardening_review.md`
- Local smoke gate (HTTP route/filter changes):
  - For work that touches HTTP route handlers, query/filter logic, or new API endpoints, run local smoke before PR publication.
  - Execute local smoke with deterministic wrappers (`make infra-provision`, `make infra-deploy`, `make infra-port-forward-start`), then run positive-path `curl` assertions per changed endpoint.
  - Positive-path filter assertions MUST use non-empty fixture/request values; empty-result-only assertions MUST NOT satisfy this gate.
  - Record evidence in `pr_context.md` as `Endpoint | Method | Auth | Result`.
  - Stop/cleanup wrappers after smoke (`make infra-port-forward-stop`, `make infra-port-forward-cleanup`).
- Publish checklist:
  - include requirement/contract coverage
  - include key reviewer files
  - include validation evidence + rollback notes

## Operational Readiness
- Logging/metrics/traces:
  - emit deterministic orphan cleanup/check diagnostics in docs sync scripts
- Alerts/ownership:
  - retain existing quality gate ownership in `quality-hooks-fast` and docs check targets
- Runbook updates:
  - remediation path remains `python3 scripts/lib/docs/sync_platform_seed_docs.py`

## Risks and Mitigations
- Risk 1: generated-consumer cleanup accidentally removes blueprint seed files.
  - Mitigation: cleanup filter uses contract-declared `required_seed_files` allowlist only.
- Risk 2: hidden template-coupled summary checks still fail in generated-consumer mode.
  - Mitigation: apply repo-mode policy consistently to runtime identity and module summary sync scripts with dedicated tests.
