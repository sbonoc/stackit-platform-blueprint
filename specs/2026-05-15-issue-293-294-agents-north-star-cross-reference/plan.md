...
1. Slice 1 — AGENTS.md template update (closes #293): Add "Architecture Invariants — Pointers" section + anti-duplication Mandatory Workflow rule to `scripts/templates/consumer/init/AGENTS.md.tmpl`. Write failing unit test for AC-001 + AC-002 (template content assertions); turn green with the template update. Run `make quality-hooks-fast`.
