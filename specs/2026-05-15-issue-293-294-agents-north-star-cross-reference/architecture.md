...
- **Template governance context** (`scripts/templates/consumer/init/AGENTS.md.tmpl`): Defines the structural contract for new consumers. Adds the Pointers section (navigation aid + anti-duplication rule) and the Mandatory Workflow rule (agent instruction). This is a one-time init artifact; existing consumers are not affected.
