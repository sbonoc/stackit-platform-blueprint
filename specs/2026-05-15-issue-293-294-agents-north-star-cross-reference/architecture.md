...
- **Template governance context** (`scripts/templates/consumer/init/AGENTS.md.tmpl`): Defines the structural contract for new consumers. Adds the Pointers section (navigation aid + anti-duplication rule) and the north_star.md MUST-read Mandatory Workflow rule (FR-002). This is a one-time init artifact; existing consumers are not affected.

- **Blueprint governance context** (`AGENTS.md` — blueprint root): Closes the auto-loading gap in the blueprint repo itself. Adds the same north_star.md MUST-read Mandatory Workflow rule (FR-007). Textual change only — no programmatic enforcement added because the heading-overlap hook targets consumer repos. The existing ADR system (lifecycle-aware, numbered) + north_star.md (settled state) + Pointers table (domain index) already provide selective decision surfacing without context-window cost; no separate decisions ledger is introduced.
...
- **Risk 4**: Blueprint's own `AGENTS.md` accumulates Mandatory Workflow rules over time. Adding FR-007 increases the rule count; if the list grows too long it becomes difficult for agents to process. Mitigation: rules are brief, distinct, and additive; no existing rule is modified. Future refactoring of AGENTS.md structure is a separate concern.
