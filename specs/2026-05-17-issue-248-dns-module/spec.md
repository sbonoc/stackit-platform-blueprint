...
- Foundation alignment: The foundation `main.tf` uses `for_each` across multiple zones via a list input — the standalone module now follows the same pattern. Zone naming (`{prefix}-dns-{hash}`) mirrors the established foundation naming strategy.
