...
|---|---|---|---|---|---|---|---|
| FR-001 | SDD-C-005, SDD-C-006 | N/A | `architecture.md` § Bounded Contexts and Responsibilities; ADR-issue-361 § Decision (4-child table) | GitHub issues `#361.1`, `#361.2`, `#361.3`, `#361.4` (filed per T-001) | T-001 manual evidence (GitHub issue links recorded in `pr_context.md`) | `specs/2026-06-18-issue-361-orchestrator-service/spec.md` § FR-001 | parent `#361` Integration AC tick log |
...
| FR-012 | SDD-C-005, SDD-C-006 | N/A | `architecture.md` § Domain layer (PredicateRegistry) + § Context D (ux-ui-designer + matrix wiring) | `#361.1` (mechanism) + `#361.4` (PERSONA.md + matrix) | `#361.1` unit tests + cross-child integration test (AC-009) | `.agents/personas/ux-ui-designer/PERSONA.md` (authored in `#361.4`); `docs/blueprint/autonomous-factory/design-contracts.md` § C3 updated rows (in `#361.4`) | C7 `expert_verdicts[]` rows include or omit `ux-ui-designer` per predicate |
...
| AC-009 | SDD-C-012, SDD-C-016 | N/A | `architecture.md` § Domain layer (PredicateRegistry) + § Context D | `#361.4` cross-child integration test `T-205` | `T-205` integration suite | spec § AC-009 | `expert_verdicts[]` audit per predicate |
