...
| FR-012 | SDD-C-005, SDD-C-006 | N/A | `architecture.md` § Domain layer (PredicateRegistry) + § Context E (usability-pragmatist + matrix wiring) | `#361.1` (predicate-registry mechanism) + `#361.5` (PERSONA.md + C3 matrix wiring + `#369` closure) | `#361.1` unit tests + cross-child integration test (AC-009) | `.agents/personas/usability-pragmatist/PERSONA.md` (authored in `#361.5`); `docs/blueprint/autonomous-factory/design-contracts.md` § C3 updated rows (in `#361.5`) | C7 `expert_verdicts[]` rows include or omit `usability-pragmatist` per predicate |
...
| FR-014 | SDD-C-005, SDD-C-016 | N/A | ADR-issue-361 § Decision (5-child table); Contract C2 (decomposed children layout) | `specs/2026-06-18-issue-361-orchestrator-service/file_children.sh` | `tests/blueprint/test_issue_361_file_children_script.py::test_file_children_first_run` + `::test_file_children_idempotent_second_run` (T-110, AC-010) | parent spec § FR-014; operator runbook in `pr_context.md` | gh API audit log of issue-create calls at parent merge |
| FR-015 | SDD-C-005, SDD-C-016 | N/A | ADR-issue-361 § Decision; AGENTS.backlog mechanical-trigger convention | `specs/2026-06-18-issue-361-orchestrator-service/add_deferred_triggers.sh` | `tests/blueprint/test_issue_361_file_children_script.py::test_add_deferred_triggers_first_run` + `::test_add_deferred_triggers_idempotent_second_run` (T-110, AC-011) | parent spec § FR-015; `AGENTS.backlog.md` diff at parent merge | backlog diff in parent merge commit |
...
| AC-010 | SDD-C-012 | N/A | parent helper script `file_children.sh` | `specs/2026-06-18-issue-361-orchestrator-service/file_children.sh` | `tests/blueprint/test_issue_361_file_children_script.py::test_file_children_first_run` + `::test_file_children_idempotent_second_run` (T-110) | parent spec § AC-010 | gh issue-create audit at parent merge |
| AC-011 | SDD-C-012 | N/A | parent helper script `add_deferred_triggers.sh` | `specs/2026-06-18-issue-361-orchestrator-service/add_deferred_triggers.sh` | `tests/blueprint/test_issue_361_file_children_script.py::test_add_deferred_triggers_first_run` + `::test_add_deferred_triggers_idempotent_second_run` (T-110) | parent spec § AC-011 | AGENTS.backlog.md diff at parent merge |
...
- Follow-up 2: `usability-pragmatist` expert raises standing roster to 9 vs ADR-issue-364 ceiling of 8. Architecture sign-off is the human approval surface; FR-012 predicate is the conditioning safety property.
