...
- [ ] (parked) proposal(issue-361-orchestrator-service): file #361.3 — RabbitMQ subscriber + OpenHands API client + work loop (trigger: after: issue-336)
      ...
      rationale: #361.3 is the \`architectural-layer: infrastructure — external-runtime clients\` boundary of the #361 5-child decomposition (canonical vocabulary per ADR-issue-337-light-decomposition-policy § Allowed boundary types). Filing deferred per Q-1 (Option B) until BOTH #335 AND #336 reach spec-complete (this entry surfaces when #336 resolves; the operator MUST also verify #335 is spec-complete before filing #361.3, since the child's OpenHands API client depends on #335's Agent Server API surface being concrete). If #336 has resolved but #335 has not, defer filing until the sibling entry on \`### after: issue-335\` also surfaces — do NOT file #361.3 against incomplete contracts (Codex P2 review of PR #372). Parent spec: ${PARENT_SPEC_PATH}. When filing, the #361.3 body MUST cite parent spec FR-016 — the PR scope MUST include \`git rm\` of \`${PARENT_SPEC_PATH}file_children.sh\` and \`${PARENT_SPEC_PATH}add_deferred_triggers.sh\` (verified by AC-012 / T-211 to be authored in #361.3). Per parent spec FR-017, the #361.3 PR body MUST cite parent #361 as \`Tracks #361\` (informational); GitHub auto-close keywords (\`Closes\` / \`Fixes\` / \`Resolves\`) MUST NOT target #361. Parent close is a deliberate human action after the Contract C4 Integration AC checkboxes are ticked.
EOF
}
