...
2. **C3 matrix schema change + matrix rows.** Extend the design-contracts.md § C3 matrix table with a 6th column \`Predicate\` (after \`Convergence mode\`); for all 8 pre-existing rows authored at \`#339\` intake, fill the new column with the literal string \`none\`; add new \`usability-pragmatist\` rows at step01/04/05/08 with the \`ui-fidelity-or-a11y\` predicate identifier consumed by \`#361.1\`'s \`PredicateRegistry\` (predicate evaluates as a DISJUNCTION: \`( has-user-facing-flow: true AND diff-touches-frontend-paths ) OR diff-touches-accessibility-relevant-paths\` — covers BOTH new user-facing flows AND accessibility-only changes to existing surfaces; rationale in parent spec FR-012 and § Clarifications Q-3).
3. **C7 \`expert_slug_blueprint\` enum widening (CROSS-CUTTING; the deal-breaker if omitted).** Author the paired amendment to \`docs/blueprint/autonomous-factory/design-contracts.md\` § C7 widening the sealed \`outcome_details.expert_verdicts[].expert_slug_blueprint\` enum from 8 to 9 (admitting \`usability-pragmatist\`) via the \`#339\` sign-off cycle. Per the F-12 clarification (amended 2026-06-19), \`expert_slug_blueprint\` is the sealed sub-enum for blueprint-baseline experts (amended only via \`#339\` sign-off); \`expert_slug_extension\` is the additive open-string sub-enum for consumer-specific experts (no sign-off required). \`usability-pragmatist\` lands in \`expert_slug_blueprint\` (not \`expert_slug_extension\`) because it is a blueprint-baseline expert shipped to all consumers, not a per-consumer overlay. Without this amendment: the orchestrator's FR-002 loader rejects the new matrix row at startup AND the C7 emitter constructs events that fail schema validation.
