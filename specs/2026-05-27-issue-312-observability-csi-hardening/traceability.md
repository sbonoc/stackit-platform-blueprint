...
| FR-007 | SDD-C-005, SDD-C-009 | N/A | Vault TF provider + `vault_kv_secret_v2` in foundation TF workspace (executes on every `stackit_foundation_apply`; guarded by `secrets_manager_enabled && observability_enabled`) | `infra/cloud/stackit/terraform/foundation/observability_vault.tf`, `foundation/versions.tf`, `foundation/outputs.tf` | `test_tf_observability_writes_vault_kv_secret` (checks `foundation/observability_vault.tf`), `test_tf_observability_declares_vault_provider` (checks `foundation/versions.tf`) | — | Secrets Manager secret path `observability/otel-credentials`; vault address from `secrets_manager_vault_address` TF output |
...
- Post-review fixes (commits fd150b7, 6263fea): 111/111 unit assertions pass (up from 108); all 11 quality gates continue to pass after review-driven changes.
