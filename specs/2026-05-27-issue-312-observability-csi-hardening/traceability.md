...
|---|---|---|---|---|---|---|---|
| FR-001 | SDD-C-005, SDD-C-013 | N/A | CSI driver ArgoCD Applications in `infra/gitops/argocd/core/` | `infra/gitops/argocd/core/secrets-store-csi-driver.yaml`, `secrets-store-csi-driver-vault-provider.yaml` | `test_csi_driver_argocd_{dev,stage,prod}_exists`, `test_csi_driver_chart_version_pinned`, `test_csi_driver_sync_wave_annotation_present`, `test_kustomization_{dev,stage,prod}_references_csi_driver` | `docs/platform/prerequisites.md` | ArgoCD sync status |
| FR-002 | SDD-C-005 | N/A | Vault provider sidecar config in CSI driver ArgoCD Application | `infra/gitops/argocd/core/secrets-store-csi-driver-vault-provider.yaml` | `test_csi_vault_provider_argocd_{dev,stage,prod}_exists` | `docs/platform/prerequisites.md` | — |
| FR-003 | SDD-C-005, SDD-C-009 | N/A | `SecretProviderClass` in observability namespace | `infra/gitops/argocd/optional/{dev,stage,prod}/observability.yaml` | `test_secret_provider_class_referenced_in_{dev,stage,prod}`, `test_secret_provider_class_namespace_is_observability` | `docs/platform/modules/observability/README.md` | `kubectl get secretproviderclass -n observability` |
...
| FR-005 | SDD-C-005 | N/A | Deprecation guard in `observability.sh` using `is_stackit_profile` + `log_fatal` + `return 1`; removed call in apply script STACKIT branch | `scripts/bin/infra/observability_apply.sh`, `scripts/lib/infra/observability.sh` | `test_apply_stackit_path_does_not_call_reconcile_runtime_secret`, `test_reconcile_runtime_secret_deprecation_guard` (asserts `is_stackit_profile` + `log_fatal`) | — | — |
| FR-006 | SDD-C-005 | N/A | Removed call in destroy script STACKIT branch | `scripts/bin/infra/observability_destroy.sh` | `test_destroy_stackit_path_does_not_call_delete_runtime_secret` | — | — |
| FR-007 | SDD-C-005, SDD-C-009 | N/A | Vault TF provider + `vault_kv_secret_v2` in foundation TF workspace (executes on every `stackit_foundation_apply`; guarded by `secrets_manager_enabled && observability_enabled`); bootstrap template seeds vault integration for new consumers | `infra/cloud/stackit/terraform/foundation/observability_vault.tf`, `foundation/versions.tf`, `foundation/outputs.tf`, `scripts/templates/infra/bootstrap/infra/cloud/stackit/terraform/foundation/observability_vault.tf`, `scripts/bin/infra/bootstrap.sh` | `test_tf_observability_writes_vault_kv_secret` (checks `foundation/observability_vault.tf`), `test_tf_observability_declares_vault_provider` (checks `foundation/versions.tf`) | — | Secrets Manager secret path `observability/otel-credentials`; vault address from `secrets_manager_vault_address` TF output |
| FR-008 | SDD-C-005 | N/A | `required_core_capabilities` updated in contract | `blueprint/modules/observability/module.contract.yaml` | `test_contract_declares_csi_driver_prerequisite` | — | — |
...
| FR-010 | SDD-C-007 | N/A | New CSI volume assertions | `tests/infra/modules/observability/test_contract.py` | `test_stackit_values_uses_csi_volume`, `test_stackit_values_no_secret_volume`, `test_secret_provider_class_referenced_in_{dev,stage,prod}`, `test_argocd_{dev,stage,prod}_uses_csi_volume_driver` | — | — |
...
- Post-review fixes (commits fd150b7, 6263fea, b699d4b): 110/110 unit assertions pass (111 after fd150b7+6263fea; −1 in b699d4b for removal of `test_tf_observability_versions_file_exists` after deleting dead `modules/observability/versions.tf`); all 11 quality gates continue to pass after all review-driven changes.
