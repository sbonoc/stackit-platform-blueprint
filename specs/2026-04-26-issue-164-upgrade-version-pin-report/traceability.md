...
|---|---|---|---|---|---|---|
| FR-001 | SDD-C-005 | Version pin diff — git read via local subprocess | `scripts/lib/blueprint/upgrade_version_pin_diff.py` | `tests/blueprint/test_upgrade_version_pin_diff.py` — mocked git subprocess returns fixture `versions.sh` | `architecture.md` §Integration Edges | `artifacts/blueprint/version_pin_diff.json` |
...
| NFR-OPS-001 | SDD-C-008 | Standalone CLI (`--repo-root`, `--source-path`, `--baseline-ref`, `--target-ref`) | `scripts/lib/blueprint/upgrade_version_pin_diff.py` | `tests/blueprint/` argparse invocation test | `spec.md` §NFR-OPS-001 | `python3 upgrade_version_pin_diff.py --help` |
