...
- FR-001 The Implementation Stack Profile in `spec.md` MUST be extended with two new fields: `has-user-facing-flow` (values: `true` | `false`) and `E2E gate classification` (values: `automated` | `manual` | `N/A`). `N/A` is the default for non-user-facing work items (`has-user-facing-flow: false`) and is always exempt from the gate. "User-facing flow" is defined as any feature exposing a form, wizard, multi-step interaction, or other interactive UI surface to an end user; backend-only or CLI-only work items set `has-user-facing-flow: false`.
...
- FR-006 The blueprint spec template (`.spec-kit/templates/blueprint/spec.md`) and the consumer spec template (`.spec-kit/templates/consumer/spec.md`) MUST be updated to seed `has-user-facing-flow` and `E2E gate classification` fields in the Implementation Stack Profile section; the consumer init template mirror MUST be synced via `sync_consumer_init_sdd_assets.py`. The `has-user-facing-flow` field MUST carry an inline HTML comment listing the intake-inference signal list (form, wizard, modal, dialog, page, screen, UI, frontend, browser, user journey, onboarding, dashboard, button, input, component, flow, frontend framework name, non-`none` frontend-stack-profile) so that any author filling in the field manually has the same checklist the step01 intake agent uses. The `E2E gate classification` field MUST carry an inline HTML comment listing the allowed values (`automated` | `manual`) and noting that `manual` is a gate violation when `has-user-facing-flow: true` and the test automation profile contains `playwright`.
...
- FR-009 `.agents/skills/blueprint-sdd-step01-intake/SKILL.md` MUST be updated to infer `has-user-facing-flow` at intake time from the issue title, description, and labels using a named signal list (keywords: `form`, `wizard`, `modal`, `dialog`, `page`, `screen`, `UI`, `frontend`, `browser`, `user journey`, `onboarding`, `dashboard`, `button`, `input`, `component`, `flow`, `checkout`, `login`, `signup`, `profile`, `settings`, `view`, `layout`, `render`, `display`; labels: `frontend`, `ui`, `ux`, `web`, `accessibility`; any frontend framework name). The inferred value MUST be annotated with an inline HTML comment identifying the signal. A frontend-stack cross-check MUST emit a clarification block in the spec if `frontend-stack-profile != none` and `has-user-facing-flow: false`. The Required Report Format MUST include a mandatory V-gate inference result line in every intake report.
...
- AC-013 [Step01 intake SKILL.md contains V-gate inference step] — verified by T-113, which MUST assert that `.agents/skills/blueprint-sdd-step01-intake/SKILL.md` contains: (a) the phrase `has-user-facing-flow` in the Discover phase; (b) at least three signal keywords from the FR-009 list; (c) the phrase `frontend-stack` cross-check in the Specify phase; and (d) `V-gate inference result` in the Required Report Format section.
- AC-014 [Absent V-gate fields produce a gate violation] — verified by T-114, which MUST assert that `_check_vgate_classification` returns at least one violation for a post-gate spec where `has-user-facing-flow` is absent from the text entirely, and at least one violation for a post-gate spec where `E2E gate classification` is absent, when the profile contains `playwright`. This directly tests the NFR-OPS-001 requirement that absence is reported as a violation rather than silently treated as `false` or `N/A`.

## Informative Notes (Non-Normative)
- Context: "V-gate" is used informally in issue #353 to denote the E2E verification gate for a user-facing feature. No such named concept previously existed in the blueprint codebase. This work item introduces the concept as two spec fields, one quality-gate check in `check_sdd_assets.py`, and a shift-left inference step in the step01 intake skill — following the same machine-enforcement pattern established by `_check_ac_format` and `_check_step03_complete_event` (issue #352). A three-value classification design (`automated` | `manual-with-target` | `manual`) was considered and rejected — see Q-3 rationale in Normative Option Decision.
