...
- AC-010 [Blueprint spec template seeds all four new fields] — verified by T-110, which MUST assert that `.spec-kit/templates/blueprint/spec.md` contains `has-user-facing-flow`, `E2E gate classification`, `E2E automation target`, and `E2E automation escalation` fields in the Implementation Stack Profile section.
- AC-011 [Consumer spec template seeds all four new fields] — verified by T-111, which MUST assert that `.spec-kit/templates/consumer/spec.md` contains all four new fields in the Implementation Stack Profile section.
...
- Context: "V-gate" is used informally in issue #353 to denote the E2E verification gate for a user-facing feature. No such named concept previously existed in the blueprint codebase. This work item introduces the concept as four spec fields and one quality-gate check in `check_sdd_assets.py`, following the same machine-enforcement pattern established by `_check_ac_format` and `_check_step03_complete_event` (issue #352).
...
- `E2E automation escalation` machine enforcement: `_check_vgate_classification` does NOT validate the value of the `E2E automation escalation` field. The field is for human-triage use only — step07 PR reviewers are responsible for verifying that a non-`none` justification is present when a work item is past its declared `E2E automation target`. No AC or test covers this field's value; its presence in the spec is the only machine-checked property (via NFR-OPS-001).
