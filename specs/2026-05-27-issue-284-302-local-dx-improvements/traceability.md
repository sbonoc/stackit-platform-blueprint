...
| FR-003 | SDD-C-009 | Shell env wins via pre-existing export restore | `scripts/lib/shell/bootstrap.sh:load_env_file_defaults` | `test_load_env_file_defaults_preserves_explicit_env` (pre-existing, ToolingContractsTests) | spec.md NFR-SEC-001 | — |
...
| FR-007 | SDD-C-008 | Fallback to `git branch --show-current` | `scripts/bin/infra/deploy.sh:patch_argocd_local_target_revision` | `test_deploy_references_argocd_local_target_revision` (fallback via `${ARGOCD_LOCAL_TARGET_REVISION:-$(git branch --show-current)}`) | spec.md FR-007 | log_info |
...
| FR-010 | SDD-C-008 | Skip guard: Application not found | `scripts/bin/infra/deploy.sh:patch_argocd_local_target_revision` | `test_deploy_patch_inside_local_profile_guard` (function body contains `kubectl get application` pre-check) | spec.md FR-010 | — |
...
| NFR-SEC-001 | SDD-C-009 | `load_env_file_defaults` existing export preservation | `scripts/lib/shell/bootstrap.sh` | `test_load_env_file_defaults_preserves_explicit_env` (pre-existing, ToolingContractsTests) | spec.md NFR-SEC-001, ADR §Consequences | — |
...
| AC-006 | SDD-C-012 | shell env wins | `bootstrap.sh` / `load_env_file_defaults` | `test_load_env_file_defaults_preserves_explicit_env` (pre-existing) | — | — |
