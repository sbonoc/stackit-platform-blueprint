...
- [ ] T-014 Write `docs/platform/accessibility/acr.md` with VPAT 2.4 structure pre-populated with all 50 WCAG 2.1 A+AA criterion rows (1.1.1 through 4.1.3) (FR-301)
- [ ] T-015 Add `acr_staleness_days: 90` to `blueprint/contract.yaml` under `spec.quality.accessibility`
- [ ] T-016 Write `scripts/bin/platform/quality/check_acr_freshness.py` implementing: file-existence check; placeholder detection; staleness check against `ACR_STALENESS_DAYS`; clear diagnostic output per NFR-OPS-001 (FR-302)
- [ ] T-017 Add `quality-a11y-acr-check` `.PHONY` target and definition to `make/platform.mk` (FR-302)
- [ ] T-018 Write `scripts/bin/platform/quality/sync_acr_criteria.py` with bundled W3C criterion list; preserves existing support/notes/evidence content (FR-303)
- [ ] T-019 Add `quality-a11y-acr-sync` `.PHONY` target and definition to `make/platform.mk` (FR-303)
- [ ] T-020 Wire `quality-a11y-acr-check` into `quality-hooks-fast` in `scripts/templates/blueprint/bootstrap/make/blueprint.generated.mk.tmpl`; run `make blueprint-render-makefile` to sync `make/blueprint.generated.mk` (FR-305)
- [ ] T-021 Update `.agents/skills/blueprint-sdd-step07-pr-packager/SKILL.md` to include ACR review checklist item (FR-304)
