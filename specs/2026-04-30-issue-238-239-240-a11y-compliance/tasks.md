...
- [ ] G-002 Confirm open questions and unresolved alternatives are `0` (Q-1 layer conditionality and Q-2 ACR integration path must be resolved)
...
- [ ] T-001 Edit `.spec-kit/templates/blueprint/spec.md`: add `NFR-A11Y-001 MUST define WCAG 2.1 Level AA compliance scope and any known exceptions.` to the standard NFR section [per Q-1 resolution]
- [ ] T-002 Edit `.spec-kit/templates/blueprint/tasks.md`: add T-Axx task block (T-A01 through T-A05); T-A02 MUST explicitly name `wcag21a` and `wcag21aa` tags and `attachTo: document.body` [per Q-1 resolution]
- [ ] T-003 Edit `.spec-kit/templates/blueprint/hardening_review.md`: add "Accessibility Gate" mandatory section with six checklist items referencing SC 4.1.2, 2.1.1, 2.4.7, 1.4.1, 3.3.1, and axe-core scan evidence [per Q-1 resolution]
- [ ] T-004 Edit `.spec-kit/templates/blueprint/traceability.md`: add `WCAG SC` column to the requirement-to-delivery mapping header row
- [ ] T-005 Update `scripts/bin/platform/quality/check_spec_pr_ready.py`: extend Accessibility Gate validation to fail on unchecked boxes in `hardening_review.md` for applicable specs [per Q-1 resolution]
...
- [ ] T-006 Add `touchpoints-test-a11y` `.PHONY` target and definition to `make/platform.mk`
- [ ] T-007 Write `scripts/bin/platform/touchpoints/test_a11y.sh` with `A11Y_BASE_URL`, `A11Y_ROUTES`, `A11Y_FAIL_ON_IMPACT` variables and `run_cmd node axe_page_scan.mjs` invocation
- [ ] T-008 Add `apps-a11y-smoke` `.PHONY` target and definition to `make/platform.mk`
- [ ] T-009 Write `scripts/bin/platform/apps/a11y_smoke.sh` wrapping `test_a11y.sh` with smoke-appropriate defaults
- [ ] T-010 Write `scripts/lib/platform/touchpoints/axe_page_scan.mjs` with `@axe-core/playwright`, explicit `['wcag2a','wcag2aa','wcag21a','wcag21aa']` tag array, JSON report output, and human-readable violation summary
- [ ] T-011 Write `scripts/lib/platform/touchpoints/axe_preset.ts` exporting `WCAG21AA_AXE_CONFIG` and `assertAxeWcag21AA`; include `attachTo: document.body` precondition note
- [ ] T-012 Extend `test-smoke-all-local` in `make/platform.mk` to include `apps-a11y-smoke`
...
- [ ] T-105 Add contract test assertions verifying:
...
- [ ] T-108 Confirm all new contract test assertions FAIL before implementation (red phase verified)

## Slice 3 — ACR scaffold and quality gate, green phase (Issue #240)
- [ ] T-014 Write `docs/platform/accessibility/acr.md` with VPAT 2.4 structure pre-populated with all 50 WCAG 2.1 A+AA criterion rows (1.1.1 through 4.1.3)
- [ ] T-015 Add `acr_staleness_days: 90` to `blueprint/contract.yaml` under `spec.quality.accessibility`
- [ ] T-016 Write `scripts/bin/platform/quality/check_acr_freshness.py` implementing: file-existence check; placeholder detection; staleness check against `ACR_STALENESS_DAYS`; clear diagnostic output per NFR-OPS-001
- [ ] T-017 Add `quality-a11y-acr-check` `.PHONY` target and definition to `make/platform.mk`
- [ ] T-018 Write `scripts/bin/platform/quality/sync_acr_criteria.py` with bundled W3C criterion list; preserves existing support/notes/evidence content
- [ ] T-019 Add `quality-a11y-acr-sync` `.PHONY` target and definition to `make/platform.mk`
- [ ] T-020 Wire `quality-a11y-acr-check` into quality hooks chain per Q-2 resolution
- [ ] T-021 Update `.agents/skills/blueprint-sdd-step07-pr-packager/SKILL.md` to include ACR review checklist item for UI-layer specs per Q-1 resolution
- [ ] T-109 Run `make test-unit-all` — confirm T-105 assertions now PASS (green phase verified)
