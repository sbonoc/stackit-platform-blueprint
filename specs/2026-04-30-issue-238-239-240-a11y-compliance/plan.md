...
- **Current status**: BLOCKED_MISSING_INPUTS — Q-1 (layer conditionality) and Q-2 (ACR integration path) must be resolved before implementation begins.

## Constitution Gates (Pre-Implementation)
- Simplicity gate: three slices of additive changes — template section additions, new Make targets and scripts, ACR scaffold and quality gate; no existing logic modified except `quality-spec-pr-ready` extension and `test-smoke-all-local` extension.
...
- Finding-to-test translation gate: the missing a11y gates are not currently a failing automated test; new contract assertions (AC-005, AC-006, AC-008, AC-009, AC-010, AC-011) translate the structural gaps into failing assertions before fixes are applied, then turn green with each implementation slice.

## Delivery Slices

### Slice 1 — SDD lifecycle template updates (Issue #238)
1. Edit `.spec-kit/templates/blueprint/spec.md`: add `NFR-A11Y-001 MUST define WCAG 2.1 Level AA compliance scope and any known exceptions.` to the standard NFR section. [conditionality per Q-1 resolution]
2. Edit `.spec-kit/templates/blueprint/tasks.md`: add T-Axx accessibility task block (T-A01 through T-A05) with explicit WCAG 2.1 AA ruleset tags and `attachTo: document.body` requirement in T-A02. [conditionality per Q-1 resolution]
3. Edit `.spec-kit/templates/blueprint/hardening_review.md`: add "Accessibility Gate" mandatory section with six checklist items (SC references: 4.1.2, 2.1.1, 2.4.7, 1.4.1, 3.3.1; plus axe-core scan evidence line). [conditionality per Q-1 resolution]
4. Edit `.spec-kit/templates/blueprint/traceability.md`: add `WCAG SC` column to the requirement-to-delivery mapping table header.
5. Update `scripts/bin/platform/quality/check_spec_pr_ready.py`: extend Accessibility Gate section validation to reject unchecked boxes for applicable specs. [conditionality per Q-1 resolution]
...
1. Add `touchpoints-test-a11y` target to `make/platform.mk` with backing `scripts/bin/platform/touchpoints/test_a11y.sh`.
2. Add `apps-a11y-smoke` target to `make/platform.mk` with backing `scripts/bin/platform/apps/a11y_smoke.sh`.
3. Write `scripts/lib/platform/touchpoints/axe_page_scan.mjs` using `@axe-core/playwright` with explicit `['wcag2a','wcag2aa','wcag21a','wcag21aa']` tag array; write `artifacts/a11y/axe-report.json`; exit non-zero on `A11Y_FAIL_ON_IMPACT` violations.
4. Write `scripts/lib/platform/touchpoints/axe_preset.ts` exporting `WCAG21AA_AXE_CONFIG` and `assertAxeWcag21AA`; include `attachTo: document.body` precondition documentation.
5. Extend `test-smoke-all-local` in `make/platform.mk` to include `apps-a11y-smoke`.
...
1. Write `docs/platform/accessibility/acr.md` with VPAT 2.4 structure; pre-populate all 50 WCAG 2.1 A+AA criterion rows (SC 1.1.1 through 4.1.3).
2. Add `quality-a11y-acr-check` target to `make/platform.mk` backed by `scripts/bin/platform/quality/check_acr_freshness.py`; wire into quality hooks chain per Q-2 resolution.
3. Add `quality-a11y-acr-sync` target backed by `scripts/bin/platform/quality/sync_acr_criteria.py` with bundled W3C criterion list.
4. Add `acr_staleness_days: 90` to `blueprint/contract.yaml` under `spec.quality.accessibility`.
5. Update `.agents/skills/blueprint-sdd-step07-pr-packager/SKILL.md` to include ACR review checklist item for UI-layer specs per Q-1 resolution.
6. Add contract tests asserting `acr.md` scaffold criterion rows, `check_acr_freshness.py` script existence, `quality-a11y-acr-check` Make target existence.
7. Run `make quality-docs-sync-core-targets` — verify ACR-related targets appear in `core_targets.generated.md`.
...
- Rollback plan: revert template diffs; revert `make/platform.mk` target additions; remove new scripts; revert `blueprint/contract.yaml` addition; revert `quality-spec-pr-ready` extension. No data migration or runtime state to reverse.
...
- Publish checklist:
  ...
  - include ACR review checklist item [per Q-1 resolution]
...
- Alerts/ownership: no alerting change; CI failure surfaces ACR staleness before merge.
...
- Risk 3: `quality-spec-pr-ready` Accessibility Gate validation rejects existing specs that lack the section → mitigation: gate only applies to newly scaffolded specs with the updated template; existing specs are consumer-owned and unaffected (no validation of existing files).
