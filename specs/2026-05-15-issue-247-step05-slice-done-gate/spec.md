...
- Success metric: (1) SKILL.md contains Guardrails #13, #14, and #15. (2) The "After All Slices Complete" table has distinct REQUIRED rows for HTTP scope and HTTP+UI rendering scope. (3) A numbered main workflow step "3. Local smoke gate" exists in SKILL.md. (4) `references/implement_checklist.md` exists with all four updated per-slice checklist items. (5) `make quality-hooks-run` passes clean.
...
- FR-006 A file MUST be created at `.agents/skills/blueprint-sdd-step05-implement/references/implement_checklist.md` containing a per-slice checklist with at least four items: (a) API response field coverage (all fields non-null/non-empty integration test); (b) Vue rendering branch enumeration (component test per branch, including fallback/degraded/error); (c) Pact consumer+provider requirement (same-slice consumer interaction; same-repo provider verification in same slice; cross-repo deferral protocol with explicit slice-done record); (d) smoke gate (non-optional, PR MUST NOT open until it passes, result table in pr_context.md).
...
- Docs contract: `.agents/skills/blueprint-sdd-step05-implement/SKILL.md` gains Guardrails #13, #14, #15 and a numbered "3. Local smoke gate" workflow step; minimum validation bundle table gains two explicit required rows; "Special cases" HTTP section is promoted to the main workflow. `.agents/skills/blueprint-sdd-step05-implement/references/implement_checklist.md` is created (file listed in `blueprint/contract.yaml` required_files but not yet on disk).
...
- AC-006 `.agents/skills/blueprint-sdd-step05-implement/references/implement_checklist.md` exists and contains checklist items for: API response field coverage; Vue rendering branch enumeration; Pact consumer+provider (with cross-repo deferral protocol); smoke gate non-optional requirement.
...
- No changes to `AGENTS.md`, `blueprint/contract.yaml`, or any Make targets.
