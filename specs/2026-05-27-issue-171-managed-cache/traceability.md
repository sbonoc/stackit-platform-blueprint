...
- Q-1 resolved (2026-05-27): `stackit_redis_instance` + `stackit_redis_credential` confirmed from provider registry. Credential attrs: `host`, `port`, `username`, `password`, `uri`. ACL via `parameters.sgw_acl`. Slice 3 unblocked (pending sign-offs for SPEC_READY).
