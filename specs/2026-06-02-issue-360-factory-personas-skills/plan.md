...
## Delivery Slices
1. **Slice 1 — Persona scaffolding + reviewer non-overlap (red→green).** Write T-101 (file existence + count) and T-105 (skill-path resolution + sign-off-role absence) as failing tests. Author the 6 implementer personas and the 4 reviewer personas to the minimum content that turns those tests green. Includes FR-001, FR-005, FR-006, FR-008, FR-011, FR-012 partial coverage.
2. **Slice 2 — New skill runbooks with `## Required Output Schema` (red→green).** Write T-102 (front-matter + jsonschema parsing) and T-103 (placeholder + secret scan) as failing tests. Author the 10 new skill `SKILL.md` files including the YAML jsonschema block, front-matter, and the FR-016 no-skill-invokes-skill discipline. Includes FR-002, FR-003, FR-005, FR-006, FR-008, FR-016.
3. **Slice 3 — DoD specifics + reviewer dimensions + cross-context-impact template (red→green).** Write T-104 (DevSecOps and Tech Lead DoD phrases) and T-106 (reviewer-dimension non-overlap + architecture-reviewer Cross-Context Impact Reporting template). Author the affected persona files to turn those tests green. Includes FR-009, FR-010, FR-013, FR-014.
4. **Slice 4 — Contract C8 enumeration + ADR + skill-invocation-by-skill ban (red→green).** Write T-107 (no skill→skill invocation directive) and the C8 enumeration assertion in T-102. Add 20 rows to `docs/blueprint/autonomous-factory/design-contracts.md` § Contract C8 § Category (c). Add the ADR `ADR-issue-360-factory-personas-skills-roster.md`. Includes FR-004, FR-007, FR-015, FR-016.
