...
| OQ-3 | The parent ADR `ADR-issue-337-persona-skill-contract.md` § Consequences states: "No persona file authored after this ADR may carry a sign-off-role name". Four personas in this roster (`architect.md`, `architecture-reviewer.md`, `security-reviewer.md`, `po-analyst.md`) contain words that match canonical sign-off-role names (Architecture, Security, Product). Two readings are possible: (a) **work-domain reading** — the name describes the work the persona does, not the sign-off it grants; FR-012 + AC-010 already enforce that no persona grants sign-off authority; or (b) **strict reading** — the Consequences sentence forbids any persona name containing a canonical sign-off-role word, requiring rename (e.g., `solution-designer.md`, `secops-reviewer.md`, `boundary-reviewer.md`, `intake-analyst.md`). | Recommend (a) work-domain reading, anchored on the Decision-section text "implies sign-off authority" — the rejected Option C example (`architect-ai` granting `ARCHITECTURE_SIGNOFF: approved`) makes clear the rejection target is grant-of-sign-off, not work-domain naming. The issue body for #360 explicitly uses these names. Capture the resolution in the spec ADR (`ADR-issue-360-factory-personas-skills-roster.md`) so it is durable. If the user prefers reading (b), rename the four files and update FR-001 / FR-011 / FR-013 / FR-014 + the ADR table. | Architecture sign-off |
