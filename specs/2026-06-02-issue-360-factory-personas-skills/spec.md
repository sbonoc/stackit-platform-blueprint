...
- FR-017 Every persona file authored by this work item MUST contain ALL of the following section headings, in EXACTLY this order: `# Persona:` (H1 with the role name), `## Role Objective`, `## Required Inputs`, `## SDD Cycle Stakes`, `## Skills Invoked`, `## Activation Triggers`, `## Collaboration & Handoffs`, `## Strict Guardrails`, `## Definition of Done (DoD)`. The 4 reviewer persona files MUST additionally contain `## Review Dimensions`, and `architecture-reviewer.md` MUST additionally contain `## Cross-Context Impact Reporting`. Each required section MUST have non-empty content (at least one non-blank line of substantive prose or bullets).
- FR-018 Every reviewer persona file (`security-reviewer.md`, `architecture-reviewer.md`, `contract-reviewer.md`, `test-coverage-reviewer.md`) MUST document the reviewer-model-heterogeneity convention from `docs/blueprint/architecture/decisions/ADR-issue-337-reviewer-model-heterogeneity.md` in a clearly labelled section (recommended placement: under `## Strict Guardrails`). The documentation MUST state that the reviewer persona MUST run on a different model family than the implementer that produced the change under review, and MUST cite the ADR by path. The runtime enforcement mechanism (orchestrator-side reviewer-rotation picker) is owned by Child B (`#361`) and is OUT OF SCOPE for this work item.
- FR-019 This work item MUST add EXACTLY ONE row to the `CLAUDE.md` § Skills slash-command table for `/blueprint-sdd-step08-agent-pr-review`. The row MUST follow the same column structure as the existing `/blueprint-sdd-stepXY-name` rows (slash command, steps, runbook path, actor). The `Actor` column value MUST be `Software Engineer` (matching the existing SDD step rows). The runbook-path cell MUST be `.agents/skills/blueprint-sdd-step08-agent-pr-review/SKILL.md`. No other rows MUST be added by this work item: the 9 other new skills are persona-invoked only and MUST NOT receive slash-command rows in this work item. (Resolves OQ-1: B, 2026-06-02.)
- FR-020 This work item MUST add a `## Required Output Schema` section containing EXACTLY ONE fenced ```yaml jsonschema``` block to each of the 8 existing SDD skill `SKILL.md` runbooks: `.agents/skills/blueprint-sdd-step01-intake/SKILL.md`, `.agents/skills/blueprint-sdd-step02-resolve-questions/SKILL.md`, `.agents/skills/blueprint-sdd-step03-spec-complete/SKILL.md`, `.agents/skills/blueprint-sdd-step04-plan-slicer/SKILL.md`, `.agents/skills/blueprint-sdd-step05-implement/SKILL.md`, `.agents/skills/blueprint-sdd-step06-document-sync/SKILL.md`, `.agents/skills/blueprint-sdd-step07-pr-packager/SKILL.md`, `.agents/skills/blueprint-sdd-traceability-keeper/SKILL.md`. Each schema MUST be self-contained valid JSON Schema (draft-07 or later) and MUST describe the structured artifact the skill returns to the orchestrator. `blueprint-version` semver front-matter (per FR-006) MUST be present on each edited file; if a file lacks it today, this work item MUST add it. (Resolves OQ-2: B, 2026-06-02.)
...
- Make/CLI contract: no new Make targets. `CLAUDE.md`'s Skills slash-command table gains EXACTLY ONE new row for `/blueprint-sdd-step08-agent-pr-review` per FR-019; the 9 other new skills are persona-invoked only and MUST NOT receive slash-command rows.
- Docs contract: `docs/blueprint/autonomous-factory/design-contracts.md` § Contract C8 § Category (c) gains 20 new rows (10 personas + 10 skills) per FR-004. `CLAUDE.md` § Skills table gains 1 new row per FR-019. The 8 existing SDD skill `SKILL.md` files listed in FR-020 each gain a `## Required Output Schema` section per FR-020.
...
- AC-014 [Persona template skeleton present and non-empty in all 10 personas] — verified by T-108, which MUST assert each persona file contains the 9 common section headings from FR-017 in EXACTLY the specified order, and that the 4 reviewer persona files additionally contain `## Review Dimensions`, and that `architecture-reviewer.md` additionally contains `## Cross-Context Impact Reporting`; AND that each required section has at least one non-blank line of content between its heading and the next heading.
- AC-015 [Reviewer model heterogeneity convention documented in each reviewer persona] — verified by T-106 (extended), which MUST assert each of the 4 reviewer persona files contains a statement that the reviewer MUST run on a different model family than the implementer that produced the change under review AND cites `ADR-issue-337-reviewer-model-heterogeneity.md` by path.
- AC-016 [CLAUDE.md Skills table contains the step08 slash-command row] — verified by T-109, which MUST assert `CLAUDE.md` contains EXACTLY ONE table row whose slash-command cell is `` `/blueprint-sdd-step08-agent-pr-review` `` and whose runbook-path cell is `` `.agents/skills/blueprint-sdd-step08-agent-pr-review/SKILL.md` ``; AND that no other row references any of the 9 other new skill names from FR-002.
- AC-017 [Backfilled `## Required Output Schema` blocks present on the 8 existing skill files] — verified by T-110, which MUST assert each of the 8 existing skill `SKILL.md` files from FR-020 contains EXACTLY ONE `## Required Output Schema` heading followed by EXACTLY ONE fenced ```yaml jsonschema``` block whose body parses as valid JSON Schema (draft-07 or later); AND that each file's YAML front-matter contains a `blueprint-version` key matching the semver pattern `^\d+\.\d+\.\d+(-[\w.]+)?$`.
...
All previously tracked open questions are resolved as of 2026-06-02 via PR #362 reviewer comments. Resolutions:

| ID | Question | Resolution | Decision date | Decided by |
|---|---|---|---|---|
| OQ-1 | Should `CLAUDE.md`'s Skills slash-command table gain a row for the new `blueprint-sdd-step08-agent-pr-review` skill? | **B — add the row in this PR.** Slash command enables developer-driven invocation during Child B implementation, matching the existing `/blueprint-sdd-stepXY-name` pattern. Scope codified in FR-019 / AC-016. The 9 other new skills remain persona-invoked-only and MUST NOT receive slash-command rows. | 2026-06-02 | Product (PR #362 comment) |
| OQ-2 | Should the existing 7 SDD step skills (`step01`–`step07`) and `blueprint-sdd-traceability-keeper` retroactively gain `## Required Output Schema` sections for parity with the new skills? | **B — backfill in this PR.** Scope codified in FR-020 / AC-017. The 8 existing skill `SKILL.md` files each gain a `## Required Output Schema` section with EXACTLY ONE fenced ```yaml jsonschema``` block. | 2026-06-02 | Product (PR #362 comment) |
| OQ-3 | The parent ADR `ADR-issue-337-persona-skill-contract.md` § Consequences states: "No persona file authored after this ADR may carry a sign-off-role name". Four personas in this roster (`architect.md`, `architecture-reviewer.md`, `security-reviewer.md`, `po-analyst.md`) contain words that match canonical sign-off-role names. | **A — work-domain reading.** The name describes the work the persona does; clause 4 is violated only when a persona TAKES a canonical sign-off action. FR-012 + AC-010 (enforced by T-105) already prevent that. Resolution captured in `docs/blueprint/architecture/decisions/ADR-issue-360-factory-personas-skills-roster.md` § Naming-convention reading for clause 4. No persona files renamed. | 2026-06-02 | Architecture (PR #362 comment) |
...
- jsonschema runtime validator — owned by Child B; this work item only authors the schemas (for the 10 new skills per FR-003 and the 8 existing skills per FR-020).
- Slash-command table updates in `CLAUDE.md` for the 9 non-step08 new skills (those skills are persona-invoked-only per FR-019).

## Potential Deferred Proposals
- (none — both prior entries resolved on 2026-06-02 by OQ-1: B and OQ-2: B; now in scope as FR-019 and FR-020.)
