...
- Rationale: Matches the established consumer reference pattern. Namespace scope limits the ACME account blast radius and keeps cert lifecycle isolation per context. `ClusterIssuer` would require cluster-admin permissions and conflicts with the blueprint's namespace-scoped policy model.
...
- Context: An existing consumer deployment uses namespace-scoped `Issuer` with `gatewayHTTPRoute` HTTP01 and production Let's Encrypt across all environments. This pattern is adopted here.
...
- Context: NFR-SEC-004 (staging ACME server for `stackit-dev` and `stackit-stage`) serves a dual purpose that is non-obvious: beyond protecting the production Let's Encrypt rate limit (5 certs/domain/week), the staging CA is not trusted by any browser or standard TLS client — which means browsers never store an HSTS entry from a staging-cert HTTPS connection. As a result, the HSTS pinning risk described in NFR-SEC-006 and architecture.md Risk 3 (1-year irreversibility) applies exclusively to `stackit-prod`. Operators and developers can freely change domains, destroy, and recreate public-endpoints deployments in `stackit-dev` and `stackit-stage` without any risk of browser clients being HSTS-pinned to a stale domain.
