...
- Tradeoff 1: Namespace-scoped `Issuer` (not `ClusterIssuer`) means each consumer namespace must create its own Issuer for consumer-specific certs. This is intentional — it matches the established consumer pattern and keeps cert lifecycle isolated per context.
