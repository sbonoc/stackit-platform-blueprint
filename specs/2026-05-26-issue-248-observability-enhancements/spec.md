...
- FR-003 MUST update `infra/local/helm/observability/otel-collector.values.yaml` to add: (a) `faro` receiver with `endpoint: 0.0.0.0:12347` and `cors.allowed_origins: ["*"]`; (b) `faro: { enabled: true, containerPort: 12347, servicePort: 12347, protocol: TCP }` port entry; (c) `faro` receiver included in traces and logs pipeline receivers.
- FR-004 MUST update `infra/cloud/stackit/helm/observability/otel-collector.values.yaml` with identical Faro receiver and port config as the local lane (this file is the baseline template for ArgoCD inline values).
- FR-005 MUST update all three ArgoCD Application inline values blocks in `infra/gitops/argocd/optional/{dev,stage,prod}/observability.yaml` to add Faro receiver (port 12347, `cors.allowed_origins: ["*"]`), the `faro` Service port declaration, and include `faro` in the traces and logs pipeline receivers.
...
- NFR-SEC-001 Faro receiver MUST accept `allowed_origins: ["*"]` by default. Faro is a telemetry-only ingress — it has no write access to STACKIT backends; a blanket origin restriction on the receiver would block legitimate browser SDKs without providing meaningful security benefit. Consumers requiring stricter CORS MUST override via `FARO_CORS_ALLOWED_ORIGINS`.
