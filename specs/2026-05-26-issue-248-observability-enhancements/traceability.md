...
| FR-003 | SDD-C-007, SDD-C-014 | n/a | Faro receiver config local lane | `infra/local/helm/observability/otel-collector.values.yaml` | `test_contract.py` — port 12347 in local values | README § Local lane | collector pod port 12347 |
| FR-004 | SDD-C-007, SDD-C-013 | n/a | Faro receiver config STACKIT lane | `infra/cloud/stackit/helm/observability/otel-collector.values.yaml` | `test_contract.py` — port 12347 in STACKIT values | README § STACKIT lane | ArgoCD Application |
| FR-005 | SDD-C-007, SDD-C-013 | n/a | Faro receiver in ArgoCD inline values | `infra/gitops/argocd/optional/{dev,stage,prod}/observability.yaml` | `test_contract.py` — port 12347 in dev/stage/prod manifests | README § STACKIT lane | ArgoCD Application |
...
| FR-018 | SDD-C-008 | n/a | ≥12 new test assertions | `tests/infra/modules/observability/test_contract.py` | pytest pass with ≥12 new assertions | — | — |
...
| NFR-SEC-001 | SDD-C-009 | n/a | Faro CORS `allowed_origins: ["*"]` | OTEL values config | `test_contract.py` — Faro receiver present | README § Security | no backend credential exposure |
