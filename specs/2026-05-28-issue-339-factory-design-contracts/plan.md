...
4. Slice 4 — Sign-off cycle. Sign-off comments from Product, Architecture, Security, Operations recorded in `spec.md` per `AGENTS.md § Sign-off Policy`. SPEC_READY flipped to `true` only when all four are recorded AND Q-4 is resolved (Q-5 already resolved — #342 filed and substituted into Contract C8 references).
