...
4. Slice 4 — Sign-off cycle. Sign-off comments from Product, Architecture, Security, Operations recorded in `spec.md` per `AGENTS.md § Sign-off Policy`. SPEC_READY flipped to `true` only when all four are recorded AND Q-4 AND Q-5 are both resolved (Q-5 requires the new Phase 1 factory-upgrade-process issue to be filed and its number substituted into Contract C8 references).
