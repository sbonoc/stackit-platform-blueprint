...
|---|---|---|---|---|---|---|
| FR-001 | SDD-C-005, SDD-C-023, SDD-C-024 | Newline-primary delimiter detection in `parse_literal_pairs()` | `scripts/bin/platform/auth/reconcile_eso_runtime_secrets.sh` — `parse_literal_pairs()` | `tests/infra/test_runtime_credentials_eso.py` — new comma-in-value test | `docs/platform/consumer/runtime_credentials_eso.md` | reconcile artifact `_reconcile_state` |
| FR-002 | SDD-C-005, SDD-C-023 | Comma-fallback branch when no `\n` in input | `scripts/bin/platform/auth/reconcile_eso_runtime_secrets.sh` — `parse_literal_pairs()` | `tests/infra/test_runtime_credentials_eso.py` — existing comma-separated test | `docs/platform/consumer/runtime_credentials_eso.md` | reconcile artifact `_reconcile_state` |
...
| FR-004 | SDD-C-010 | Error message references both formats | `scripts/bin/platform/auth/reconcile_eso_runtime_secrets.sh` — `record_reconcile_issue` call | `tests/infra/test_runtime_credentials_eso.py` — error path test | none | `artifacts/infra/runtime_credentials/_reconcile_issues.json` |
...
| AC-002 | SDD-C-012 | Backward compat — comma-separated simple values | `scripts/bin/platform/auth/reconcile_eso_runtime_secrets.sh` | `test_dry_run_reconcile_writes_success_state_and_renders_source_secret` passes | none | none |
