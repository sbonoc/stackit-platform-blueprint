...
| FR-007 | SDD-C-007, SDD-C-013 | n/a | STACKIT otel-collector Helm values | `infra/cloud/stackit/helm/observability/otel-collector.values.yaml` | `test_contract.py` — YAML content check (exporters, projected Secret volume mount, file-provider references, spanmetrics connector) | README | ArgoCD sync status |
