...
- Metrics/logging/tracing updates: OTEL Collector Helm values (`infra/cloud/stackit/helm/observability/otel-collector.values.yaml`) configure three signal exporters — `prometheusremotewrite` (metrics), `loki` (logs), `otlp/stackit` (traces) — each using BasicAuth with credentials read via the OTC file config provider (`${file:/etc/otel/secrets/username}`, `${file:/etc/otel/secrets/password}`) from a projected volume mount of `blueprint-observability-auth` at `/etc/otel/secrets`. Signal pipelines are explicit and non-overlapping. Health check extension exposed on port 13133 for K8s readiness probes. Spanmetrics connector bridges traces→metrics pipeline.
...
Proposals promoted to in-scope during this PR are marked `(implemented)`.

- Proposal 1 (implemented): spanmetrics connector — `connectors.spanmetrics` added to `otel-collector.values.yaml` and all three ArgoCD Application inline values; traces pipeline fans out to `spanmetrics` connector; metrics pipeline receives from `spanmetrics`; histogram buckets and HTTP dimensions configured. Test assertion added to `OtelCollectorValuesTests`.
