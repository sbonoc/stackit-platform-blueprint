...
- Risk 2 (otel-collector values complexity): Configuring BasicAuth exporters in the OpenTelemetry Collector Helm chart requires careful projected Secret mount + `${file:/etc/otel/secrets/...}` wiring. Mitigation: follow agentic-graphrag Langfuse pattern; validated locally before STACKIT.
