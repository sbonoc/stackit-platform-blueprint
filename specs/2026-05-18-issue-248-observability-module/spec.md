...
- FR-002 MUST add three helper functions to `scripts/lib/infra/observability.sh`: `observability_metrics_push_url()`, `observability_logs_push_url()`, `observability_traces_push_url()`. On the STACKIT lane each MUST source from the corresponding foundation output via `stackit_foundation_output_value_or_default`. On the local lane Secret reconciliation is not invoked (local lane uses the `argocd_manifest_plus_helm` driver, not `foundation_contract`), so these functions return empty string on local lane.
- FR-003 MUST add `observability_api_key()` helper to `scripts/lib/infra/observability.sh` returning empty string on all lanes. The credential is delivered exclusively via the `blueprint-observability-auth` K8s Secret — not the state file. The `api_key` state key exists to satisfy the module contract output declaration and to signal that auth is provisioned; it is intentionally empty to avoid persisting credentials in shell-layer artifacts.
...
- FR-006 MUST update `scripts/bin/infra/observability_destroy.sh` to: (1) call `run_manifest_delete` on the ArgoCD optional manifest path to remove the otel-collector Application resource before the credential Secret is deleted; (2) call `observability_delete_runtime_secret()` to remove the `blueprint-observability-auth` K8s Secret; in the `foundation_reconcile_apply` case.
- FR-007 MUST create `infra/cloud/stackit/helm/observability/otel-collector.values.yaml` containing an OpenTelemetry Collector configuration with: OTLP gRPC+HTTP receivers (ports 4317, 4318), batch processor, and three exporters — `prometheusremotewrite` targeting `${file:/etc/otel/secrets/METRICS_PUSH_URL}` with BasicAuth from `${file:/etc/otel/secrets/username}`/`${file:/etc/otel/secrets/password}`, `loki` targeting `${file:/etc/otel/secrets/LOGS_PUSH_URL}` with BasicAuth, and `otlp/stackit` targeting `${file:/etc/otel/secrets/TRACES_PUSH_URL}` with BasicAuth. The values MUST include `extraVolumes`/`extraVolumeMounts` mounting `blueprint-observability-auth` Secret at `/etc/otel/secrets` (read-only) so the collector pod reads credentials from files via the OTC file config provider — NOT as environment variables.
- FR-008 MUST update `infra/gitops/argocd/optional/{dev,stage,prod}/observability.yaml` to include an ArgoCD `Application` resource (in addition to the existing metadata `ConfigMap`) that deploys the `open-telemetry/opentelemetry-collector` Helm chart in the `observability` namespace using a per-environment inline `values:` block (following the established blueprint ArgoCD pattern; `infra/cloud/stackit/helm/observability/otel-collector.values.yaml` serves as the template/baseline). The Application MUST reference the platform AppProject and set `syncPolicy.automated.selfHeal: true`.
- FR-009 MUST update `scripts/bin/infra/observability_smoke.sh` to add, on the STACKIT lane (guarded by `is_stackit_profile`): non-empty checks for `logs_endpoint`, `metrics_endpoint`, `traces_endpoint` (must start with `http`); and a presence check for the `api_key` key (key must exist; value is intentionally empty).
