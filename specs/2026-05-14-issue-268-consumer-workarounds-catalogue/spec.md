...
- AC-011: Given a synthetic issue body containing `## Automated Workaround Catalogue Entry` with all four fields populated (`affected_version`, `action_kind`, `applies_when`, `action_content`) and the `workaround-report` label applied, the GitHub Actions scaffolder workflow produces: (a) a correctly named action file at `workarounds/v<affected_version>/` containing the verbatim `action_content`, and (b) a manifest entry stub in `manifest.yaml` with all eight fields present (`id` from issue number, `upstream_issue` as issue URL, `title`, `applies_when`, `action_kind`, `action_path`, `apply_phase` as TODO comment, `landed_in: null`), and (c) a draft PR linking to the originating issue.
