...
- [x] T-023 Author `docs/blueprint/architecture/decisions/ADR-issue-337-c7-emission-mechanism.md` per FR-019 with `Status: approved` from inception (round-10 amendment under meta-ADR sign-off envelope per traceability Follow-up 9) — the sealed two-emitter rule (orchestrator #333 phase-boundary events + webhook handler #336 GitHub-observable events; LiteLLM/OpenHands/workspace pods/personas/skills are NON-emitters; idempotency `event_id = sha256(ticket_id|phase|rerun_round|emitter)` (terminology unified on `ticket_id` per #339 round-13 to match the sealed C7 schema field name); local exemption via `LIFECYCLE_EVENT_TARGET=local`) (Slice 2)
