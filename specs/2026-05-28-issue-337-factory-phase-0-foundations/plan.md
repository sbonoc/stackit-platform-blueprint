...
## Delivery Slices
1. Slice 1 — Open Decisions resolution gate. Before authoring any ADR content, resolve Q-1 through Q-7 in `spec.md` per the Draft PR review cycle (Step 2 of the SDD workflow). For each Open Decision, record the selected option, rationale, and any deferred follow-up under `### Open Decisions` in the relevant ADR's body or in `spec.md § Informative Notes`. Slice 1 exit criterion: `spec.md` shows `Open clarification markers count: 0` and SPEC_READY transitions to `true` after all four canonical sign-offs are recorded.
2. Slice 2 — Author the ten content ADRs. For each FR-001 through FR-010, create the matching `docs/blueprint/architecture/decisions/ADR-issue-337-<topic>.md` with `Status: approved` (status flipped to `approved` only after the four sign-offs in Slice 8). Each ADR includes Context, Decision Drivers, Options Considered, Decision, Consequences, References sections; each ADR cites its #339 contract dependency (e.g., FR-005 cites #339 NFR-SEC-001 verbatim; FR-001 cites #339 Contract C3); each ADR carries the extensibility-tier classification statement from its matching FR. Slice 2 exit criterion: ten ADR files exist; all link back to the meta-ADR; `make docs-build` shows them in the ADR index.
...
8. Slice 8 — Validation and sign-off. Run `make quality-sdd-check`, `make docs-build`, `make docs-smoke`, `make quality-hardening-review`. Record results in `evidence_manifest.json` via `make spec-evidence-manifest`. Record sign-offs from Product, Architecture, Security, Operations per `AGENTS.md § Sign-off Policy` (canonical phrases verbatim). Flip the ten content ADRs and the meta-ADR `Status: approved`. Slice 8 exit criterion: AC-001, AC-003, AC-004, AC-005, AC-007, AC-008, AC-011 all satisfied.
