...
- FR-001 The orchestrator (#361) MUST emit, on every panel-dispatched C7 event, the extension field `outcome_details.token_usage` containing per-expert token counts — keyed by `expert_slug` — with sub-fields `input_tokens` (integer ≥ -1) and `output_tokens` (integer ≥ -1) for each dispatched expert. Values ≥ 0 represent actual counts; -1 is the normative sentinel for unavailable usage data (see NFR-REL-001).
...
- FR-003 The orchestrator (#361) MUST emit, on the step08 close-out C7 event, the extension field `outcome_details.ticket_token_summary` containing three keys with explicit computation rules: (1) `total_input_tokens` (integer ≥ 0) — sum of `token_usage[expert_slug].input_tokens` across all panel-dispatched phase events for the `ticket_id`, treating sentinel -1 as 0; (2) `total_output_tokens` (integer ≥ 0) — sum of `token_usage[expert_slug].output_tokens` across all panel-dispatched phase events, treating sentinel -1 as 0; (3) `total_expert_step_instantiations` (integer ≥ 0) — count of individual expert-slug entries in `token_usage` maps across all panel-dispatched phase events for the `ticket_id`.
- FR-004 #361's spec MUST declare a per-ticket budget expressed as a cost ceiling (P95 $X USD) and a separate token ceiling (P95 N input tokens). The audit predicate MUST be implemented as the `audit-cost --ticket <id>` sub-command of `scripts/bin/sdd/c7_emit.py` (Q-1 resolved, PR #371): it reads the step08 `ticket_token_summary` from the persisted JSONL, compares against two named Python constants (`COST_CEILING_USD` and `TOKEN_CEILING_INPUT`), and emits `outcome: rejected` with `rejection_reason: cost-ceiling-exceeded` on breach. The initial constant values MUST be `$5 USD` and `500K input tokens`; a calibration comment MUST mark both for update after the first 3 autonomous runs.
