...
- Risk 3 — Per-expert model assignment cost: With panel sizes of 2–4 per step (and 7 at step08), token cost rises versus the stage-persona model. Mitigation: the matrix caps panel sizes; the heterogeneity ADR amendment encourages assigning cheaper models (Haiku) to high-volume, low-stakes experts (e.g., Documentation Discipline) and stronger models (Opus) only to high-stakes experts (Security Paranoid, Boundary Hawk).
