...
- FR-003 `ADR-issue-364-expert-persona-model.md` MUST define EXACTLY ONE OF the three convergence patterns as the default (`parallel-then-merge`) and MUST enumerate the steps where each non-default pattern applies (`sequential-lens` reserved for `step05`; `structured-disagreement` reserved for `step03` and `step08`). The ADR MUST state that conflicting `block` verdicts SHALL surface at the existing two human gates (spec sign-off, PR merge) and SHALL NOT introduce a new gate.
...
- FR-006 The 10 skill runbooks under `.agents/skills/blueprint-*/SKILL.md` shipped by #360 / PR #362 MUST be re-homed onto this branch with the following modifications: (a) the `Actor` section MUST cite the orchestrator and the expert panel rather than a named stage-persona (e.g., "tech-lead persona"); (b) the `Composition` section MUST cite the orchestrator's dispatch table as the binding mechanism; (c) `blueprint-agent-pr-review/SKILL.md` MUST accept a panel-input parameter naming the experts to consult and MUST produce a per-expert verdict array in its `## Required Output Schema`.
