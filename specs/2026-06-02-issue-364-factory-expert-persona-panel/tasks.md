...
- [ ] T-112 Author shell-based AC-012 assertion (per PERSONA.md, grep body prose excluding H1 / frontmatter for other-slug whole-word matches; assert zero matches across the 8 personas).
- [ ] T-113 Author shell-based AC-013 assertion (`git ls-files .agents/personas/*.md` returns zero matches for the 10 old flat persona file basenames; consumer/ subdirectory excluded).
- [ ] T-114 Author shell-based AC-014 assertion (loop over `.agents/skills/blueprint-sdd-step0[1-8]-*/SKILL.md`; assert `## Required Output Schema` present; zero stage-persona slug matches; `expert_verdicts` OR `expert_slug` present for panel ≥ 2 steps).
- [ ] T-115 Author shell-based AC-015 assertion (`grep -cE "^## Role and Philosophy" AGENTS.md` exits with count 1; that line contains `operator-default`; body within section contains `Persona precedence`; no second `^## (Role|Persona)\b` heading elsewhere).
- [ ] T-009 Delete the 10 old flat persona files from `.agents/personas/` (FR-011) in the same commit that introduces the 8 expert-slug directories. Preserve `.agents/personas/consumer/`.
- [ ] T-010 Maintain the Cherry-Pick Ledger in `pr_context.md` per FR-006 procedural contract (one row per runbook; commit SHA from PR #362 + stripping-edit commit SHA on this branch).
