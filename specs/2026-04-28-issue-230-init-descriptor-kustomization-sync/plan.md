...
3. Slice 3 — **Contract drift guard + docs**: add a unit test asserting the on-disk init scope listed in `blueprint/contract.yaml` covers every path the init force-reseed actually writes (AC-004). Document the expanded force-init blast radius in `docs/blueprint/architecture/decisions/ADR-2026-04-28-issue-230-init-descriptor-kustomization-sync.md` § Consequences and the consumer-facing regression note in `docs/platform/consumer/troubleshooting.md` (no `docs/blueprint/upgrade/release_notes.md` — that file is not a repo convention; versioning is ADR-based). Promote ADR `Status: proposed` → `Status: approved` once Architecture sign-off lands.
