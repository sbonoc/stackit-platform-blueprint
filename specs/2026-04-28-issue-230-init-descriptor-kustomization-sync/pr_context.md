...
## Deferred Proposals
- Proposal 1: `proposal(issue-217): extract _assert_descriptor_kustomization_agreement as shared module helper for future smoke scenario reuse`. **Parked — trigger: on-scope: blueprint** — confirmed by reviewer at Step 07 (2026-04-28). The FR-003 smoke fixture extension landed in this PR via `template_smoke.sh`'s pre-seed hook; that hook reuses the existing assertion through the same code path and does not introduce a new caller of `_assert_descriptor_kustomization_agreement`. Extracting a shared helper today would be premature abstraction. Will resurface automatically the next time a work item is scaffolded with `on-scope: blueprint`. Backlog entry: `AGENTS.backlog.md` (existing parked row; rationale already captures this).
